...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the ATHENA_HOME= match — see
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m athena gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``athena update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Athena's detached watcher as well would escape the
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (ATHENA_HOME,
    ...
    if sys.platform == "win32":
        try:
            from athena_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ATHENA_HOME) the windowless
        # base interpreter needs to import athena_cli.  Empty on POSIX, where
...
def _athena_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ATHENA_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_athena_home_from_systemd_unit(system: bool) -> None:
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ATHENA_HOME", "").strip()
    if not unit_home:
        return
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (athena_cli/gateway.py restart
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # athena_constants cached from before this symbol existed. See
            # _reload_athena_constants() in athena_cli/managed_uv.py.
...
def _athena_home_for_target_user(target_home_dir: str) -> str:
    ...
    current_athena = (
        Path(current_athena_raw).expanduser()
        if current_athena_raw
        else get_athena_home()
    )
    ...
    # different ATHENA_HOME into the generated service unit.
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute Python interpreter and ``-m athena``, so module
...
def _systemd_watchdog_service_fields(
    athena_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(athena_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], athena_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Athena-managed Node under ``$ATHENA_HOME/node`` goes first when it
    ...
    *athena_root* is the Athena home the unit will run against. System units
    ...
    for directory in iter_athena_node_dirs(athena_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Athena home is known — probing here would stat the calling
    ...
    if system:
        ...
        # the remapped athena_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(athena_home) if athena_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m athena{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m athena{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ATHENA_HOME={athena_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ATHENA_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo athena gateway … --system``, ATHENA_HOME is often stripped
    # and falls back to ``/root/.athena``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ATHENA_HOME comparisons
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ATHENA_HOME-sync chokepoint (adopts the unit's pinned home before
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ATHENA_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Existing system units already pin ATHENA_HOME; adopt it before any
    ...
    # ``sudo athena gateway install --system --force`` would bake /root/.athena
...
def systemd_start(system: bool = False):
    ...
    # ATHENA_HOME sync happens inside refresh_systemd_unit_if_needed's
...
#      background process (the `nohup athena gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m athena [--profile X] gateway run --replace` argv.
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.athena/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See athena-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_athena_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_status(deep: bool = False):
    ...
    # Athena PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Athena PID file).
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # the env vars ``ATHENA_GATEWAY_MAX_STARTS`` /
    # ``ATHENA_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ATHENA_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ATHENA_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``athena gateway run`` enters through this CLI wrapper, not through
