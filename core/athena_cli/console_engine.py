...
This module backs ``athena console`` and is intentionally narrower than the
full Athena CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'athena doctor'")
        and last.startswith("Run 'athena setup'")
    ):
        return text.rstrip()
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `athena "):
        return ""
    return summary
...
# dashboard opens a fresh AthenaConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("athena_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["AthenaConsoleEngine", list[str]], str]:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["AthenaConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `athena {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `athena {usage}`?",
        )
...
class AthenaConsoleEngine:
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Athena component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Athena logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Athena configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Athena Console."""

        extracted = {
            "version": (
                "athena_cli.subcommands.version",
            ...
            "prompt-size": (
                "athena_cli.subcommands.prompt_size",
            ...
            "insights": (
                "athena_cli.subcommands.insights",
            ...
            "security": (
                "athena_cli.subcommands.security",
            ...
            "import": (
                "athena_cli.subcommands.import_cmd",
            ...
            "plugins": (
                "athena_cli.subcommands.plugins",
            ...
            "pairing": (
                "athena_cli.subcommands.pairing",
            ...
            "webhook": (
                "athena_cli.subcommands.webhook",
            ...
            "profile": (
                "athena_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Athena configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "athena_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("athena_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "athena_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("athena_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("athena_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Athena Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Athena Console.")
...
def _doctor(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _config_migrate(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: AthenaConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: AthenaConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from athena_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: AthenaConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="athena_cli.subcommands.profile",
...
def _cron_status(_engine: AthenaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from athena_cli.cron import cron_status
...
def _cron_pause(_engine: AthenaConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from athena console")
...
def run_console_repl(
    *,
    ...
    engine = AthenaConsoleEngine()
    if interactive:
        print("Athena Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("athena> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
