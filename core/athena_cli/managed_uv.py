...
The Python backing the install is different: it is shared by every Athena
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.athena-runtime/python``. A
...
from athena_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``athena update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under ATHENA_HOME
    caches the last successful self-update time.
    """
    try:
        from athena_constants import get_athena_home

        stamp = get_athena_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_athena_constants():
    """Re-execute ``athena_constants`` from disk and return the fresh module.

    ``athena update`` imports ``athena_constants`` from the OLD checkout,
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Athena generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Athena-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``athena update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``athena update`` repairs the runtime (issue class:
