...
def _check_dispatcher_presence(
    ...
    ``athena_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different ATHENA_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=athena_home, use_cache=False
        )
    except Exception:
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "See the Kanban guide included with the Athena source."
        ),
    )
    # --- global --board flag ---
    ...
    p_create.add_argument(
        ...
        dest="toolsets",
        help=(
            "Worker capability set (repeatable), or --toolset auto to select "
            "from the task title/body before startup. Omit to use the assignee "
            "profile defaults."
        ),
    )
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $ATHENA_PROFILE or 'user')")
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # athena_cli.kanban_db, because children can import DB mutators directly.
    ...
    with board_scope:
        ...
        # every `athena kanban repair` into "could not initialize database"
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            toolsets=getattr(args, "toolsets", None) or None,
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `athena kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # athena_cli/goals.py. Unpacking fewer raises
