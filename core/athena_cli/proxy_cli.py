...
The top-level command is ``athena egress``.  Note that the inbound OAuth
reverse-proxy command (``athena proxy``) lives elsewhere in
``athena_cli/main.py`` — different direction, different purpose.
"""
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    ``athena egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``athena proxy`` subparser (which uses dest='proxy_command').
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `athena secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.athena/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `athena egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.athena/.env for discovery.[/dim]"
            )
    ...
    # explicitly requested rotation.  This prevents re-running `athena
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Athena sandbox.  They will start "
    ...
    # `athena egress reload` apply future ruleset changes without a
    ...
    # rotation), running `athena egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]athena egress start[/cyan] manually before "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `athena egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`athena egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                ...
                "project, or switch back via `athena egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``athena egress restart`` instead: the daemon reads real credentials
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]athena egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `athena egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.athena/.env`` into ``os.environ``.

    ``athena egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.athena/.env`` (which the agent
