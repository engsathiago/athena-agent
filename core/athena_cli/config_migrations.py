...
``athena_cli.config.migrate_config``. Each step is a function
...
``athena_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``athena_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``athena_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``athena_cli.config`` (e.g.
``patch("athena_cli.config.read_raw_config", ...)``) keep working, because
...
#: ATHENA_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    """Return the live ``athena_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_athena_home = _c.get_athena_home
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `athena config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `athena config set`"
                )
...
# behavior set it to true explicitly via `athena config set`.
