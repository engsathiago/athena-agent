...
# commands at execution time when ``_ATHENA_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Check status:  athena cron status", Colors.DIM))
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec athena "
                        "athena cron ...`). Fix ownership to match the "
...
def cron_command(args):
    ...
    print("Usage: athena cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
