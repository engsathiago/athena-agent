...
This module is imported by ``athena_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no athena_cli.config,
...
``athena_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(athena_root: str) -> bool:
    ...
    active_profile = os.path.join(athena_root, "active_profile")
    try:
...
def container_mode_may_be_active() -> bool:
    ...
    if athena_home:
        ...
        return (
            ...
            and active_profile_may_override_home(athena_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".athena")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ATHENA_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    print("Run 'athena version' for update status.")
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    subcommand + the ATHENA_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
