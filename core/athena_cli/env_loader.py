...
# load_athena_dotenv can call _sanitize_env_file_if_needed multiple times
...
def _known_athena_env_keys() -> set[str]:
    """Return the combined set of known Athena env-var keys.
    ...
    ``load_athena_dotenv()`` calls.
    """
    from athena_cli.config import _EXTRA_ENV_KEYS
    from athena_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Athena process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Athena keys absent from ``.env``.
    ...
    parent Athena process injects and that silently change *which provider
    ...
    ``tests/athena_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_athena_dotenv()'s .op.env bootstrap: the 1Password
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``athena_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_athena_dotenv(
    *,
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Athena keys
    ...
    #   EnvironmentFile=-/path/to/.athena/.op.env
    ...
    # TERMINAL_ENV=docker left in ~/.athena/.env (e.g. written by an older
    # `athena setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_athena_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``athena_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process ATHENA_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_athena_dotenv(athena_home=...)`` call would bridge the wrong
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Athena reads
    ...
    owns the once-per-ATHENA_HOME guard, the post-apply ASCII
    ...
    if not cfg:
        ...
        # on its next load_athena_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_athena_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `athena model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `athena` startup, so populating the
    # shared cache here lets main.py's early bridge and athena_logging reuse
...
def _process_athena_home() -> Path:
    """The ATHENA_HOME the shared config cache is keyed to."""
    try:
