...
SPOTIFY_DOCS_URL = "the Spotify guide included with the Athena source"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "the OAuth-over-SSH guide included with the Athena source"
...
def get_anthropic_key() -> str:
    ...
    ``~/.athena/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.athena/.env over os.environ so a deliberate key rotation
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Athena
    ...
    # the user initiated inside Athena (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "athena_pkce", "manual"}
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.athena/.env isn't silently
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``ATHENA_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Athena at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``ATHENA_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Athena persists a Codex 429's ``reset_at`` locally and freezes the
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``athena auth status``, …),
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Athena
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # ATHENA_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``athena auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``athena auth list`` would show
