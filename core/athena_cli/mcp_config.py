...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``athena mcp login`` and
    ``athena mcp reauth`` so both behave identically for a single server.
    """
    ...
    #
    # force_interactive_oauth: `athena mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Athena desktop / agent-
...
def cmd_mcp_reauth(args):
    ...
    ``athena mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``athena mcp reauth --all`` discovers every ``auth: oauth`` server in
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("athena mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
