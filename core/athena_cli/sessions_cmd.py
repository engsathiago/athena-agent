"""``athena sessions`` command — extracted from ``athena_cli/main.py``.
...
Helpers that stay in ``athena_cli.main`` (``get_athena_home``,
...
``athena_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    athena sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        from athena_state import workspace_key as _ws_key
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_athena_home() / "session-exports"
                    )
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_athena_home() / "session-exports"
        ...
        # `athena sessions prune`: no time window and no filters at all
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`athena sessions optimize` later to reclaim freed space.)")
