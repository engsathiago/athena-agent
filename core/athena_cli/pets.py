...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: athena pets install boba")
            return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: athena pets select {slug}")
    return 0
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: athena pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: athena pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: athena pets select " + active.slug)
