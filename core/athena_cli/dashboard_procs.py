...
``_detect_concurrent_athena_instances``) are lifted verbatim. References to
helpers that STAY in ``athena_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``athena_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``athena_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``athena_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``athena-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``athena serve`` as
    ...
    patterns = [
        ...
        # The headless backend (`athena serve`) is the same long-lived server
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``athena dashboard`` / ``athena serve`` processes.
    ...
    When ``restart_managed`` is true (the ``athena update`` path), a detected
    ``athena-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``athena-serve.service``) has its owning unit
    ...
    # caller asked for restarts (the `athena update` path) — `--stop` must
    ...
    #    remote backend (athena serve) under Restart=on-failure never comes
...
def _detect_concurrent_athena_instances(
    ...
    for shim in _m()._athena_exe_shims(scripts_dir):
        try:
