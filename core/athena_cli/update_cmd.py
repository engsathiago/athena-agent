"""Athena update pipeline — extracted from ``athena_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``athena_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``athena_cli.main`` reference — so existing call sites and test monkeypatches
that target ``athena_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``athena_cli.main``.
...
Imports are one-way: ``athena_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``athena_cli.main`` reference.

    Lets callers keep patching ``athena_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``athena_cli.main`` -> ``athena_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``athena update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Athena must be able to import immediately after an
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``athena update`` can be driven by a
    ...
    probe = (
        ...
        # The root set is injected from athena_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('athena_'):\n"
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         see the Curator guide included with the Athena source"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `athena update`.
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``athena_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `athena update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``athena_constants.py``
...
def _update_via_zip(args):
    ...
    if not OFFICIAL_REPO_URL:
        print("✗ Athena's archive update channel is not configured.")
        _m().sys.exit(1)
    zip_url = f"{OFFICIAL_REPO_URL.removesuffix('.git')}/archive/refs/heads/{branch}.zip"
    ...
    try:
        ...
        print(
            "  Re-run `athena update` to retry; if the agent won't start, "
            "reinstall from a newer Athena source checkout"
        )
        _m().sys.exit(1)
    finally:
    ...
    try:
        ...
        _state_path = get_athena_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_athena_home())
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This checkout has no official Athena upstream configured.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added the configured Athena upstream.")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Configure an official Athena repository before adding upstream.")
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Athena' editable-install extras or
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `athena update` is exactly the step
...
def _npm_lockfile_changed(athena_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `athena
    ...
    try:
        ...
        cache_file = athena_root / f".npm_lock_hash_{cache_key}"
...
def _update_node_dependencies() -> list[str]:
    ...
    # Athena profile using this checkout. Keep one per-checkout cache under the
    # shared Athena root rather than rerunning npm once per named profile.
    ...
    # hung. The chatty npm-deprecation noise during `athena update` comes from
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.athena/logs/update.log`` only, never the terminal.

    During ``athena update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if not git_dir.exists() and (_m().PROJECT_ROOT.parent / ".git").exists():
        # A distribuição Athena mantém o pacote Python em core/, enquanto o
        # checkout Git fica um nível acima. Comandos executados em core/
        # descobrem esse repositório normalmente; aceite a mesma estrutura na
        # verificação explícita.
        git_dir = _m().PROJECT_ROOT.parent / ".git"
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``athena-acp`` launcher next to the ``athena`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``athena update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``athena-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Athena as not installed even when it is.

    The shim simply delegates to the sibling ``athena`` launcher with the
    ...
    ``athena-acp.exe`` already resolves) and wherever a ``athena-acp`` is
    already present next to the ``athena`` command.  Unwritable directories
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ATHENA_HOME under
      ``backups/`` (restorable via ``athena import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from athena_constants import
        # get_athena_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from athena_cli.config import get_athena_home as _get_home
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``athena update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``athena update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run athena from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.athena-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".athena-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The athena.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m athena_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``athena
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m athena_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "athena_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Athena processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Athena Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append("    athena update")
    lines.append("  (or use `athena update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``athena update``
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``athena-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("athena-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``athena update`` never touched them, so installs created before the
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``athena update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_athena_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `athena serve` backend) will keep .pyd
    ...
    # the athena.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  Run ./install.sh from a newer Athena source checkout.")
            sys.exit(1)
    ...
    try:
        ...
        # marker survives and the next ``athena`` launch finishes the install
        ...
        # symbols in athena_constants or lazy_deps. The dependency install
        ...
        if not import_ok:
            ...
            print("    Re-run ./install.sh from a newer Athena source checkout.")
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `athena desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "athena", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Athena-managed Node on PATH:
                # when `athena update` runs inside the desktop updater chain
                # (Desktop → athena-setup → athena update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from athena_constants import with_athena_node_path

                _build_env = with_athena_node_path()
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_athena_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # Self-heal the athena-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Athena on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("athena-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `athena update`
                # must stay fast; `athena computer-use install --upgrade`
