...
Scope of the repair is deliberately narrow. Athena only upgrades an npm that
lives inside its **own** managed Node tree (``$ATHENA_HOME/node``), installing
...
and their other projects; Athena never modifies those. When the failing npm is
one of those foreign installs, Athena instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    ``$ATHENA_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ATHENA_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Athena-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Athena-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$ATHENA_HOME/node`` (reusing a healthy
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Athena-managed install, or
    ...
    if prefix is not None:
        # Athena owns this npm — upgrade it in place. Only an npm-range
