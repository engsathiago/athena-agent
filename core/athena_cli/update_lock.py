...
* ``athena update`` from a terminal,
* the dashboard's Update button (``POST /api/athena/update`` →
  ``_spawn_athena_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``athena-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``athena update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ATHENA_HOME>/.athena-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``athena update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Athena is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``athena-setup`` binary under ``~/.athena`` is only refreshed by a full
  installer run (``copy_self_to_athena_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Athena is still running") on every
  GUI update forever, with no Athena process actually running.
"""
...
# Set by an orchestrating updater (the Tauri `athena-setup --update` flow) to
# its own pid before spawning `athena update` as a child stage. The parent
...
# to show "Athena is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Athena home (never the context-local profile override):
    the Rust updater resolves ``$ATHENA_HOME`` or the platform default, and the
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``athena update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``athena-setup`` binaries
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `athena update` as a stage): we run under ITS claim
