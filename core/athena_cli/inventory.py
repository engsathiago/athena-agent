...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``athena_cli/moa_cmd.py``.
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `athena model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Athena.
    """
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`athena_cli.model_switch.list_picker_providers`) so the
