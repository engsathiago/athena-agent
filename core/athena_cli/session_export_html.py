"""
HTML Export generator for Athena sessions.
...
def generate_multi_session_html_export(sessions: List[Dict[str, Any]]) -> str:
    ...
    for s in sessions:
        sid = str(s.get("id", "N/A"))
        escaped_sid = _escape_html(sid)
        title = s.get("title") or "Athena Session"
    ...
    return HTML_TEMPLATE.format(
        page_title="Athena Session Export" if is_multi else _escape_html(sessions[0].get("title", "Athena Session")),
