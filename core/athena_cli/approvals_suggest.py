...
Athena has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.athena/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, athena self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    ...
    from tools.approval import (
        _rewrite_resolved_athena_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_athena_home(command))
...
def approvals_command(args) -> int:
    ...
    print(
        ...
        "\n"
        "Run `athena approvals suggest -h` for details."
    )
    return 1
