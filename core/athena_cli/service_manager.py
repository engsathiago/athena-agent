...
class S6ServiceManager:
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock as athena when
            # this script starts as root. Never chown or unlink athena-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is athena-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded athena-owned at stage2
