...
``CommandDef.execute`` (athena_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/athena_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``athena_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_athena_home()
