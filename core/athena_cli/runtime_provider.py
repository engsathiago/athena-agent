...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ATHENA_INFERENCE_PROVIDER`` equivalent). When neither
...
def resolve_runtime_provider(
    *,
    ...
    from athena_cli.config import is_provider_enabled, load_config
