...
def _build_full_manifest(
    ...
    By default, this keeps Athena on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Athena in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
...
def slack_manifest_command(args) -> int:
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "athena slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"athena slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "athena slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "athena slack manifest: long description must be at most "
