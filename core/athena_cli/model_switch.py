...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``athena model`` wizard — not a
    ...
    while ``athena model`` still live-probed the full ``/v1/models`` list.
...
# Match only the real Nous Research Hermes 3 / Athena 4 chat families.
...
async def resolve_display_context_length_async(
    ...
    ``athena_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    # athena_cli.runtime_provider.resolve_runtime_provider; without the strip,
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Compared against athena_id / mdev_id (section 1), pid / athena_slug
    ...
    # the optional Athena model catalog so
    ...
    for athena_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # same athena_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if athena_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Athena may not route yet.
        ...
        if not is_runtime_provider_routable(athena_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(athena_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        athena_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
    ...
    for pid, overlay in ATHENA_OVERLAYS.items():
        ...
        if athena_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or athena_slug.lower() in _excluded:
            continue
        ...
        total = len(model_ids)
        if athena_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Athena writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Athena's own writer uses (mirrors section-4 grouping).
            ...
            # ``athena model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``athena model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``athena model``. Pin a dict catalog with
