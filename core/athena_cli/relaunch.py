...
def resolve_athena_bin() -> Optional[str]:
    ...
    Priority:
      ...
      3. ``None`` → caller should fall back to ``python -m athena``.
    ...
    ``athena.exe`` on PATH or the ``sys.executable -m athena``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m athena``) or a ``.cmd`` batch file, and both
