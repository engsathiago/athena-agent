...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        ...
        Path.home() / ".local" / "bin" / "athena-acp",
        Path.home() / ".local" / "bin" / "athena-agent",
        Path("/usr/local/bin/athena"),
        Path("/usr/local/bin/athena-acp"),
        Path("/usr/local/bin/athena-agent"),
    ]
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m athena` so we don't depend on an `athena`
    #    wrapper that may be half-removed mid-uninstall.
    athena_invocation = [_sys.executable, "-m", "athena", "--profile", name]
...
def _print_uninstall_dry_run(*, project_root: Path, athena_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        ...
        if _is_default_athena_home(athena_home):
...
def _perform_uninstall(
    *,
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  Run Athena's Windows installer from a newer source checkout.", Colors.DIM))
        else:
            print(color("  Run ./install.sh from a newer Athena source checkout.", Colors.DIM))
        print()
