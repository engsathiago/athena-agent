...
def _preserve_athena_home_path(path: str | Path) -> str:
    """Render Athena-owned paths under the configured ATHENA_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\athena`` as a symlink/junction to
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "athena", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``ATHENA_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m athena [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``athena gateway run``.
    ...
    lines.append(f'set "VIRTUAL_ENV={_preserve_athena_home_path(venv_dir)}"')
    pythonpath_entries = [
    ...
    prog_args = [python_exe_path, "-m", "athena"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_athena_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_athena_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('ATHENA_HOME')}) = {_quote_vbs_string(athena_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('ATHENA_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_athena_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_athena_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    athena_home = str(Path(get_athena_home()))
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, athena_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    project_root = _preserve_athena_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    athena_home = str(Path(get_athena_home()))
    profile_arg = _profile_arg(athena_home)

    argv = [python_exe, "-m", "athena"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "VIRTUAL_ENV": _preserve_athena_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_athena_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m athena gateway run`` directly — NOT
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from athena_cli.config import get_athena_home
        print(f"    type {Path(get_athena_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_athena_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from athena_cli.config import get_athena_home

    athena_home = Path(get_athena_home())
    print()
    print("Next steps:")
