...
def is_noninteractive() -> bool:
    ...
    ``ATHENA_NONINTERACTIVE=1`` (see ``athena_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ATHENA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ATHENA_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, athena_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Athena cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    athena model            (pick any provider/model)")
        print_info("    athena setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'athena tools' → Speech-to-Text")
            )
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    print_info("   Guide: see the README included with Athena")
    print()
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `athena egress setup` then `athena egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `athena egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'athena-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $ATHENA_HOME/bin is never on PATH, so a bare
            # which() misses the uv Athena installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'athena-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.athena/logs/tool_calls.log (gateway only)")
...
def _setup_telegram():
    ...
    managed_onboarding = bool(os.environ.get("TELEGRAM_ONBOARDING_URL", "").strip())
    if managed_onboarding:
        print_info("How would you like to create your Telegram bot?")
        print()
        print_info("  [1] Automatic")
        print_info("      Use your configured Telegram onboarding service.")
        print_info("  [2] Manual")
        print_info("      Create a bot via @BotFather and paste the token.")
        print()
        choice = prompt("Choice [1/2]", default="1")
    else:
        print_info("Create a Telegram bot via @BotFather and paste its token.")
        choice = "2"
    token = None
    setup_result = None
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: see the messaging documentation in the Athena source")
    print()
    ...
    print_info("   See the webhook routing guide included with the Athena source")
    print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: athena gateway install --system")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Athena profile and are not uploaded.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      athena setup telemetry — just local shared metrics
