...
late-binding seam in :mod:`athena_cli.web_deps` so tests that
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps athena_cli.profiles.export_profile /
# import_profile (the same machinery behind `athena profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_athena_home() / "profile-exports"
        try:
