...
web_server - reached via the late-binding seam in :mod:`athena_cli.web_deps`
...
from athena_cli.web_deps import late, LateState
...
_spawn_athena_action = late("_spawn_athena_action")
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the ATHENA_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_athena_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_athena_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.athena_home == flow_home
