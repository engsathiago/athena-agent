...
# Allow-list for ``export_profile("default")``: when ATHENA_HOME equals the
...
# into the export. The set below identifies the *known Athena profile
# artifacts* at the root of ATHENA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``athena_constants``.
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, athena subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
# Real wrappers are a few hundred bytes of shell; the needle (``athena -p X``)
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ATHENA_HOME``.
    """
    try:
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Athena *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Athena backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ATHENA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `athena -p <canon> profile
    ...
    athena_markers = ("athena_cli.main", "athena-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Athena process: either an entrypoint marker in argv, or
            # a resolved executable named `athena`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_athena = (
                any(marker in joined for marker in athena_markers)
                or exe_name == "athena"
                or exe_name.startswith("athena")
            )
            if not is_athena:
                continue
            ...
            # ...or by ATHENA_HOME env pointing at this profile dir.
            if not bound:
                try:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_athena_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ATHENA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Athena file the
      user may have lying alongside ATHENA_HOME (#58394).
