...
Why this exists
---------------
``athena_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``athena_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``athena_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``athena_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("athena_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import athena_cli.web_server as mod  # type: ignore[no-redef]
    return mod
