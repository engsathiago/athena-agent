...
Extracted verbatim from ``athena_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from athena_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``athena model`` custom flow collects.
    api_key: str = ""
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under ATHENA_HOME.
    output: str = ""
