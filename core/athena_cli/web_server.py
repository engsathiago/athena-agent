...
Usage:
    python -m athena web          # Start on http://127.0.0.1:9119
    python -m athena web --port 8080
"""
...
from athena_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
def _warm_gateway_module() -> None:
    ...
    ``athena_cli.gateway``. But the first WS connection and its initial
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import athena_cli.gateway eagerly *before* the lifespan yield so the
...
from athena_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Athena
...
def _count_status_active_sessions() -> int:
    ...
    while another Athena process is writing to it.
    """
    from athena_state import DEFAULT_DB_PATH, SessionDB
...
# ATHENA_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ATHENA_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Athena credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ATHENA_HOME resolution, so it blocks these trees wherever
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Athena inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``athena:fs:writeText`` hardening: the path is
...
# gate + path hardening as /api/fs). Logic lives in ``athena_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from athena_cli import web_git as _web_git  # noqa: E402
...
from athena_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from athena_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ATHENA_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_athena_home())
            )
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Athena Cloud
...
def _spawn_athena_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``athena`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "athena", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _ATHENA_GATEWAY=1. Inheriting it makes a spawned `athena gateway restart`
...
async def gateway_drain(request: Request):
    ...
    active (no ``ATHENA_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
from athena_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from athena_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from athena_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from athena_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/athena is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (ATHENA_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        athena_home / f"{name}.json",
        athena_home / name / "config.json",
    ):
...
def _catalog_provider_env_metadata() -> dict:
    ...
    for d in provider_catalog():
        ...
        # Vertex — a `athena model` provider — would be invisible in the desktop
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Athena to Google Chat via Cloud Pub/Sub.",
        "docs_url": "",
    },
    "wecom": {
...
# athena_cli/setup_hidden_env.py. Shared with the `athena setup gateway`
# wizard so the surfaces ask for the same things.
from athena_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # ATHENA_HOME contextvar override (#56986 / #69143), so the profile's
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the ATHENA_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized ATHENA_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_athena_oauth_file,
        )
    except ImportError:
        read_athena_oauth_credentials = None  # type: ignore
        _get_athena_oauth_file = None  # type: ignore
    ...
    if athena_creds and athena_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "athena_pkce",
            "source_label": f"Athena PKCE ({_get_athena_oauth_file() if _get_athena_oauth_file else None})",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_athena_oauth_file
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `athena auth add xai-oauth`: first credential may become
            ...
            # prior ``athena auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``athena model`` re-login path in _login_xai_oauth).
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    from athena_state import SessionDB, _default_db_path, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``athena serve`` — it runs neither the
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (ATHENA_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.athena``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    ATHENA_HOME outside the profiles tree) has no profile-dir equivalent, so
...
from athena_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from athena_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Athena home so the job's store,
...
from athena_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from athena_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `athena auth remove`) can seed again.
        # Mirrors the `athena auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``athena auth remove`` uses: each source cleans up its
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"athena-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_athena_action`` tracks one process/log per name, so a shared
...
from athena_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from athena_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
from athena_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from athena_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
from athena_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _resolve_chat_argv(
    ...
    ``ATHENA_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    """
    from athena_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Athena TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides ATHENA_HOME afterwards.
    from tools.environments.local import build_subprocess_env
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ATHENA_DASHBOARD_WS_HOST`` env var — wins always. Operators
...
# ---------------------------------------------------------------------------
# /api/console — safe Athena Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Athena CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="athena-console",
                )
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Athena Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Athena Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    """
    # `athena serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("ATHENA_SERVE_HEADLESS") == "1"
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``ATHENA_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Athena Teal canvas, then
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_athena_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``ATHENA_HOME``.
    """
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as athena_cli.plugins):
    1. User plugins:    ~/.athena/plugins/<name>/dashboard/manifest.json
    ...
    # ATHENA_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_athena_home() / "plugins", "user"),
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.athena/plugins/``) ship with the CWD and are
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ATHENA_DESKTOP_READY_FILE`` and waits for this JSON.
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ATHENA_SERVE_HEADLESS``), so
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Athena backend listening on {host}:{actual_port}")
            else:
