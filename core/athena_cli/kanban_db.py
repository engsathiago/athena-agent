...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``athena_constants.VALID_REASONING_EFFORTS`` plus
...
class Task:
    ...
    # ``athena_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``athena_cli.sqlite_safe_read``).
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see athena_cli.sqlite_safe_read), so it must only run once the board has
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``athena kanban repair`` can report and choose
    its own exit code.
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``athena kanban attach``) so name-sanitisation, the size cap, and the
...
def _default_spawn(
    ...
    *,
    ...
    # read on the board and in `athena kanban log` — it is not a conversation
    ...
    # A worker must NEVER boot the interactive TUI: an inherited ATHENA_TUI=1
