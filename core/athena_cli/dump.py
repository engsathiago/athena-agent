...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.athena/.env.
    ...
    exports. ``athena debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.athena/.env: a managed
