...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``athena tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`athena_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
# Config-only capabilities: they appear in `athena tools` for provider/API-key
...
# provider+model picker like `athena model` — NOT this single-key prompt, so
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ATHENA_HOME/bin is never on PATH, so a bare which()
    # misses the uv Athena installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    falling through to the full upstream installer. ``athena update`` sets
    ...
    a further ~600s wait on Windows). ``athena computer-use install
    ...
    ``athena update`` already prints a contextual line before its update
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `athena computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `athena update` (require_confirmed_update=True) keeps the
    ...
    # `athena computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: athena computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `athena update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `athena update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.athena/logs/update.log — write straight to it so
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $ATHENA_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Athena installed for exactly this toolchain.
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/engsathiago/athena-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.athena/config.yaml")
            else:
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``athena-discord``, which contains those
...
#:
#: Saving ``athena tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[athena-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``athena tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"athena-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (athena-acp, athena-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``athena-discord``) is an opt-in to the platform's native default-off
    ...
    # is invalid (e.g. a migration or hand-edit left `athena` instead of
    # `athena-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `athena update`/`athena doctor`.
...
def _agent_browser_installed() -> bool:
    ...
    # The install hook runs in a spawned ``athena tools post-setup`` process,
...
# Kept in sync with the dashboard selects (athena_cli/web_server.py
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``athena model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``athena model`` aux-task
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `athena model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: see the README included with Athena", Colors.DIM))
    print()
