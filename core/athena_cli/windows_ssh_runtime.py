...
def _root() -> Path:
    ...
    # helper's token, including custom ATHENA_HOME layouts.
...
def process_state(pid: int, creation_time_ns: int, athena_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    # argv[0] is either the athena exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs athena>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "athena"]
        or (len(argv) > 2 and argv[1] == "-c" and "run_module('athena'" in argv[2])
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves athena_cli's own location and prepends its parent, because the
    ...
    # the athena_cli package parent must be explicit -- the launcher resolves it via
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('athena',run_name='__main__',alter_sys=True)"
    )
