...
The ``athena`` console entry point is ``athena_cli.main:main``.  Importing
``athena_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``athena_cli.env_loader``, ``yaml`` via ``athena_cli.config``, ...).  In
...
a corrupted venv.  ``athena_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``athena_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``athena acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``athena_cli.main``
...
def recover_if_needed(
    ...
    recovery marker from a prior ``athena update`` exists AND an import probe
