...
the tool registry or launching Athena (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``athena-cli`` to the non-existent ``athena``. ``resolve_toolset('athena')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``athena-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``athena`` instead of ``athena-cli``),
