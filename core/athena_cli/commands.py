...
class CommandDef:
    ...
    # Names a key in ``athena_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``athena_cli.slash_exec.run_execute`` and apply only
...
# Telegram allows up to 100 BotCommands. Athena ships ~50 built-in commands;
...
#   - topup: the billing/balance surface; reached via /athena topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /athena moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /athena debug on Slack.
#   - egress: Docker-only proxy status; reachable as /athena egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /athena init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /athena version on
...
#   - diff: git working-tree diff; reached via /athena diff on Slack so it
...
#     /athena update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_ATHENA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update"})
