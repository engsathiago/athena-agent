...
- athena config get      - Print a resolved configuration value
- athena config set      - Set a specific value
- athena config unset    - Remove a user configuration value
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ATHENA_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # ATHENA_TOOL_PROGRESS variant is fully unsupported since the v12 config
    ...
    "ATHENA_ACP_AUTH_METHOD",
    "ATHENA_ACP_AUTO_APPROVE",
...
# from `nix run` / `nix profile install` (which don't set ATHENA_MANAGED).
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Athena was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    3. ATHENA_MANAGED env / .managed marker (NixOS managed mode)
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``engsathiago/athena-agent`` image bakes a ``docker``
    ...
    # detect Nix installs that don't set ATHENA_MANAGED (e.g. ``nix run``,
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull engsathiago/athena-agent:latest"
...
Athena Agent runs as a published image (engsathiago/athena-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull engsathiago/athena-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate athena-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm engsathiago/athena-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/engsathiago/athena-agent/tags
...
from athena_constants import get_athena_home, get_process_athena_home  # noqa: F811,E402
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ATHENA_HOME, upgrading legacy empty templates.
...
# ensure_athena_home(). Only successful passes are recorded, so a raised
...
def ensure_athena_home():
    ...
    After the first successful pass for a given ``ATHENA_HOME`` we only re-run
    ...
    ``get_athena_home()`` and therefore re-run for the new path.
    """
    ...
    if key in _ATHENA_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``athena profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ATHENA_HOME would resurrect an empty skeleton
...
from athena_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Athena' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Athena versions) may write
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "smart_model_routing",   # written by the setup wizard (athena_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (athena_cli/setup.py)
    "known_plugin_toolsets", # written/read by athena_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, athena send) so
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "athena update / athena -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        # The per-version steps live in athena_cli.config_migrations as a
    ...
    # `athena update`, but only the version bump is persisted.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_athena_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    backends) — while display paths (``athena status``/``dump``) read ``name``
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_athena_home``, tui profile override, multi-profile probes).
    """
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_athena_dotenv()
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Athena writes itself."""
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ATHENA_CUSTOM_`` prefix keeps the result a valid POSIX name
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.athena/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.athena/.env`` over ``os.environ``.

    Used for Athena-managed credentials where a deliberate edit to ``.env``
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `athena cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``athena config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``athena config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``athena config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (athena config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``athena config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``athena config set model gpt-5.6-sol`` when model already
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `athena config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: athena config get <key> [--json]")
            print()
            print("Examples:")
            print("  athena config get model")
            print("  athena config get terminal.backend")
            print("  athena config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: athena config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: athena config unset <key>")
            print()
            print("Examples:")
            print("  athena config unset model")
            print("  athena config unset terminal.backend")
            print("  athena config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  athena config get <key>          Print a resolved config value")
        print("  athena config set <key> <value>   Set a config value")
        print("  athena config unset <key>        Remove a config value")
