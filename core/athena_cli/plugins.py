...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Athena' shipped guidance lives in the evidence-based
...
class PluginContext:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Athena core) are trusted by default —
        ...
        try:
            ...
            athena = athena_authorization_override(
                "plugin.override", tool_name, tool_name=tool_name
            )
        except Exception:
            athena = None
        if athena is not None:
            return bool(athena["allowed"])
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_athena_dotenv()`` startup — after
        ``~/.athena/.env`` loads, before Athena reads credentials — when
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # all ~20 of them added several seconds to every `athena`
            # invocation — including plain `athena chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Athena ships
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``athena plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`athena plugins
...
def resolve_pre_tool_block(
    ...
    if details.action in {"block", "approve"}:
        try:
            from agent.athena_security_bridge import athena_authorization_override
        ...
        if athena is not None:
            if athena["allowed"]:
                return None
            return f"BLOCKED by Athena policy: {athena.get('reason')}"
