...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``athena memory setup`` — if the update-time refresh only reinstalled
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``athena update`` heals the active memory provider
    ...
    # data volume (ATHENA_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like athena-cli).
