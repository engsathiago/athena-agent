...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see athena_cli/model_setup_flows.py).
...
# In-repo fallback for the model Athena silently lands on when the user never
...
# ``athena model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in athena_cli/web_server.py and
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Athena lands on when none is selected.
            desc = "default"
        else:
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return athena-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; athena's picker expects
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # env var, so it must change the fingerprint too or `athena config set
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Athena' native Anthropic adapter. The live catalog may
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `athena moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
