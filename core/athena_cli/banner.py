...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `athena ... > file`) raises
...
# Repositório oficial usado apenas para verificar se existe uma versão nova.
# A aplicação instalada continua sendo atualizada pelo instalador da
# distribuição, preservando o estado separado em ATHENA_HOME.
_UPSTREAM_REPO_URL = "https://github.com/engsathiago/athena-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/engsathiago/athena-agent"
...
def _is_official_ssh_remote(url: str | None) -> bool:
    return bool(_OFFICIAL_REPO_CANONICAL) and _is_ssh_remote(url) and (
...
def check_for_updates() -> Optional[int]:
    ...
    # ATHENA_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/athena/update/check` endpoint short-circuits docker the same way
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Athena checkout. Cached per-process. A release URL is returned only when
    Athena's release channel has been explicitly configured.
    """
    ...
    repo_dir = repo_dir or _resolve_repo_dir()
    if repo_dir is None or not _RELEASE_URL_BASE:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]· Athena[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or athena setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]· Athena[/]")
