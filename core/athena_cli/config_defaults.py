"""Default configuration data for Athena Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from athena_cli/config.py. Must not import from athena_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # ``ATHENA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "checkpoints": {
        ...
        # ``athena checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # Single-query (``athena -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Athena' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); athena =
                                      # Athena' compression threshold triggers
    ...
    "display": {
        ...
        # model — see athena_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ATHENA_HOME-relative YAML files/directories
        ...
        # CLI, TUI, and desktop app. Manage with `athena pets`. Disabled until
        ...
        "pet": {
            ...
            # get_athena_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ATHENA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
    ...
    # "Hey Athena" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey athena",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_athena" (the bundled, works-out-of-the-box default) OR a
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.athena/cache/delegation/
    ...
    "moa": {
        ...
        # <athena_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "cron": {
        ...
        # ATHENA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # Optional remotely hosted model catalog manifest. Athena ships with its
    # in-repo catalog and does not contact an inherited upstream by default.
    "model_catalog": {
        "enabled": False,
        "url": "",
    ...
    "gateway": {
        ...
        # internal ATHENA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.athena/sessions/sessions.json entirely.
        ...
        # the ATHENA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.athena.gateway` that defenses 1-2 don't
    ...
    "sessions": {
        ...
        # the default of ``athena sessions prune``.
        "retention_days": 90,
        ...
        # user runs `athena sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `athena update` prints a one-line notice with
        ...
        # ~/.athena/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ATHENA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ATHENA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <ATHENA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``athena backup``-style zip
        #     of ATHENA_HOME into <ATHENA_HOME>/backups/, restorable with
        #     ``athena import``. Can add minutes on large homes. This is the
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.athena/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Athena may use this encrypted
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.athena/.env (or the shell)
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Athena also calls
    ...
    #
    # Configure with `athena egress setup`.  Disabled by default — the rest of
    # Athena works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.athena/bin/ on
    ...
    # Athena Desktop (Electron app) launch options. These only affect
    # `athena desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ATHENA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Athena mints short-lived tokens from. Falls back to "
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Athena' wake word engine (optional; openWakeWord is the free default)",
    ...
    # ATHENA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ATHENA_TOOL_PROGRESS_MODE for backward
    ...
    # ATHENA_TOOL_PROGRESS is fully unsupported since the v12 config support
