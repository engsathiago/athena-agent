...
# Two lines of inline path math so ``python athena_cli/main.py`` (script
# mode — sys.path[0] is athena_cli/, not the repo root) can import the
# canonical helpers; everything else lives in athena_cli._startup_fast.
...
# a prior ``athena update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from athena_cli.env_loader import ...`` / ``from athena_cli.config import
...
# package directory, so if IT can't import, nothing else in athena_cli can
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ATHENA_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``athena … chat -q`` with stdio on a pipe. This
...
def _active_profile_may_override_home_fast(athena_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(athena_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``athena --version`` before config/logging imports."""
...
from athena_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
try:
    ...
    # yaml.load — the SAME parse then serves athena_logging's
...
def _relative_time(ts) -> str:
    ...
    in :mod:`athena_cli.timefmt` so lightweight consumers don't have to
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $ATHENA_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Athena installed,
    ...
    # athena_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_athena_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Athena-managed
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``athena … chat -q`` with stdout
...
def select_provider_and_model(args=None):
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``athena model``
    ...
    # matching against athena_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``athena model`` flow, then route aux tasks
    to them here.
    """
    from athena_cli.config import load_config
...
def cmd_sync(args):
    ...
    if sub == "status":
        ...
        if not status.get("logged_in"):
            ...
            print(
                ...
                "ATHENA_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="athena sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="athena sync now")
...
def cmd_project(args):
    ...
    from athena_cli.projects_cmd import projects_command
...
def cmd_skin(args):
    ...
    from athena_cli.skin_cmd import skin_command
...
def cmd_import(args):
    """Restore an Athena backup from a zip file."""
...
# Update pipeline extracted to athena_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (athena_cli.main._cmd_update_impl,
# athena_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from athena_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in ATHENA_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``athena update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``athena update``
    ...
    ``athena`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``athena update``
    ...
    ``athena_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ATHENA_HOME."""
    from athena_constants import get_athena_home
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Athena-managed install, or a freshly provisioned managed npm when
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "athena_cli" / "web_dist" / "index.html"
    try:
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Athena.exe
...
# Athena.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Athena.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Athena.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 athena-setup.exe (and thus
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Athena.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Athena is damaged and can't be opened" — and macOS TCC forgets every
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``athena desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ATHENA_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "cwd", None):
        ...
        env["ATHENA_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ATHENA_DESKTOP_DISABLE_GPU=... athena desktop` keeps working.
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Athena-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → athena-setup →
            # athena update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `athena update` path).
            ...
            if not source_mode:
                ...
                # success on a Athena.exe Windows cannot load (truncated PE from
...
# Dashboard process-hygiene helpers extracted to athena_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on athena_cli.main.<name> keep resolving unchanged.
from athena_cli.dashboard_procs import (  # noqa: F401
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "athena serve",
            "athena_cli.main serve",
            "athena_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Athena installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``athena-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/athena-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but athena launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``athena update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``athena.exe`` launch always has the launcher as an
...
def _windows_running_athena_launcher_locked() -> bool:
    """True when a venv ``athena*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _athena_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``athena.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``athena.exe``.
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``athena.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``athena.exe`` shim is locked during ``athena update``, or when uv/distlib
    skips a launcher write. The symptom is ``athena-agent.exe`` and
    ``athena-acp.exe`` present but ``athena.exe`` missing, so ``athena`` drops
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m athena <command>"
        )
    else:
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.athena)")
            else:
        ...
        from athena_cli.profiles import profile_exists, validate_alias_name
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} athena dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    if choice == "2":
        ...
        print(
            ...
            "ATHENA_DASHBOARD_OAUTH_CLIENT_ID into Athena's private .env file.\n"
            "  See the dashboard guide included with the Athena source."
        )
        sys.exit(0)
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under $HOME/.athena/desktop-ssh: a
    # literal "~/.athena/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of ATHENA_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_athena_home(): a non-default sticky profile (or any ATHENA_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_athena_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".athena" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ATHENA_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``athena dashboard`` that
...
def cmd_dashboard(args):
    ...
    # ATHENA_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `athena dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ATHENA_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ATHENA_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ATHENA_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ATHENA_SERVE_HEADLESS below.
    if os.environ.get("ATHENA_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ATHENA_WEB_DIST", "")
    ...
    if (
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ATHENA_HOME is explicitly pinned to the
    ...
    if _headless_backend:
        ...
        os.environ["ATHENA_SERVE_HEADLESS"] = "1"
    elif "ATHENA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ATHENA_WEB_DIST
        ...
        # ATHENA_WEB_DIST is set without --skip-build: the build is skipped
        ...
        if not (_dist_root / "index.html").exists():
            ...
            sys.exit(1)
        # Write the expanded path back: web_server reads ATHENA_WEB_DIST raw
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``athena <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``athena photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ATHENA_YOLO_MODE is set before ANY
...
def main():
    ...
    # If the checkout changed since the last launch (athena update, manual
    ...
    # `athena proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``athena proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # approvals command  (parser built in athena_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: athena_cli/subcommands/import_agent.py)
    # =========================================================================
    ...
    # =========================================================================
    # skin command  (parser built in athena_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in athena_cli/subcommands/console.py)
    # =========================================================================
    ...
    # cmd_sessions lives in athena_cli/sessions_cmd.py (main.py decomposition).
    ...
    # --yolo: set ATHENA_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``athena egress start`` refusing when credential_source=bitwarden
