...
``athena_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `athena dashboard` / `athena serve` cmdlines and SIGTERM them directly —
    # the same path `athena update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Athena web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Athena web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # browser. It exists so the Athena Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        ...
        description=(
            "Run the Athena backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports ATHENA_SERVE_HEADLESS=1). `dashboard` leaves it
