...
Cloned from ``athena_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    athena sync status                 show gate/opt-in/head state
    athena sync pull                   pull and materialize opted-in skills
    athena sync push                   push opted-in skills
    athena sync now                    reconcile: pull then push
    athena sync enable <skill>         opt a skill into sync
    athena sync disable <skill>        opt a skill out of sync
    athena sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    athena sync propose <skill>        share a skill with your organisation
