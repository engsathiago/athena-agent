...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Athena containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `athena update`"
    return (
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. ATHENA_TOOL_PROGRESS_MODE is still read by the gateway as a
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `athena doctor --fix`, or "
...
def run_doctor(args):
    ...
    try:
        import sqlite3
        from athena_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Athena already refuses to enable WAL on fresh DBs.
    ...
    if env_path.exists():
        ...
        # matches athena_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            from athena_cli.config import load_env as _load_env_depr
            from athena_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if state_db_path.exists():
        try:
            ...
            from athena_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'athena doctor --fix' "
                        "(or 'athena sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'athena-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `athena acp --setup-browser` installs agent-browser into the
        # Athena-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_athena_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(ATHENA_HOME / "node" / "bin")
            or _which_in(ATHENA_HOME / "node")
        )
        _legacy_ab = _which_in(ATHENA_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `athena update` wiped node_modules (issue #48521).
            check_warn(
    ...
    try:
        from athena_cli.config import read_user_config_raw as _read_raw_mem
