...
_PACKAGE_SCHEMA_VERSION = "athena.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def record_model_call(
        ...
        self.record_counter(MODEL_ROUTE_METRIC, dimensions, athena_version)
    ...
    def counter_snapshot(self) -> list[dict[str, Any]]:
        ...
        with self._connection() as connection:
            rows = connection.execute(
                ...
                ORDER BY period_start, athena_version, metric_name, dimensions_json
                """
            ).fetchall()
        return [
            {
                ...
                "athena_version": row["athena_version"],
    ...
    def _pending_period_count(self) -> int:
        with self._connection() as connection:
            row = connection.execute(
                ...
                FROM (
                    SELECT period_start, athena_version
                    FROM counter_aggregates
                    WHERE value > packaged_value
                    GROUP BY period_start, athena_version
                )
                """
            ).fetchone()
    ...
    def _create_package_in_transaction(
        ...
        period_row = connection.execute(
            """
                ...
                FROM counter_aggregates
                WHERE value > packaged_value
                ORDER BY period_start, athena_version
                LIMIT 1
                """
        ).fetchone()
        ...
        rows = connection.execute(
            """
                ...
                """,
            (period_value, period_row["athena_version"]),
        ).fetchall()
        ...
        payload = {
            ...
            "resource": {"athena_version": period_row["athena_version"]},
