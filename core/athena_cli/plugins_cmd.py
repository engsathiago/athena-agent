...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``athena setup`` / ``athena plugins disable basic`` can park the plugin
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Athena core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``athena_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.athena/plugins``. Include package metadata here
    so ``athena plugins list`` can show and enable them.
    """
    from athena_cli.plugins import ENTRY_POINTS_GROUP
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``athena plugins enable web/firecrawl`` cleared
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    ...
    # Sibling of the CLI ``athena plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``athena_cli.main`` never covers them. After a
