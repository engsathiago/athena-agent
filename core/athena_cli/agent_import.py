...
Usage:
    ...
    athena import-agent claude-code --dry-run # preview only, no changes
    athena import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``athena claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_athena.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ATHENA_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ATHENA_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ATHENA_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ATHENA_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ATHENA_HOME/skills/codex-imports/<name>/
...
deliberately via ``athena setup`` or config.yaml.
"""
...
# Same entry delimiter as the Athena memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`athena config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_athena.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Athena command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Athena glob.
...
def import_agent_command(args) -> None:
    ...
    if not source_dir.is_dir():
        ...
        print_info("Specify a custom path: athena import-agent "
