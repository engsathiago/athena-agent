...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Athena-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.athena/sessions/``.  Without
...
class AIAgent:
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `athena --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Athena-Session-Id header) and must not escape
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Athena' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Athena model request this cancels only that request;
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Athena YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``ATHENA_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.athena/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``athena setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from athena_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from athena_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = athena_xai_default_headers()
        else:
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from athena_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated athena_logging._session_context (a
            ...
            # ATHENA_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from athena_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
