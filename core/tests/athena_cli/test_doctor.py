...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull engsathiago/athena-agent:latest" in hint
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Athena-managed node bin (~/.athena/node/bin), not on PATH or in
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_athena_tool_progress_warning_says_unsupported_since_floor(self):
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"ATHENA_TOOL_PROGRESS": "true"})
        )
        ...
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"ATHENA_TOOL_PROGRESS_MODE": "all"})
        )
