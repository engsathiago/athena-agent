...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("athena_cli.main._is_windows", return_value=True), \
             patch("athena_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("athena_cli.main._run_quarantined_install") as mock_install:
            from athena_cli.main import _verify_console_scripts_installed
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("athena_cli.main._is_windows", return_value=True):
            names = {path.name for path in main_mod._athena_exe_shims(fake_scripts_dir)}
