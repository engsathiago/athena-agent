...
SCHEMA_PATH = (
    ...
    / "observability"
    / "schemas"
    / "athena.shared_metrics.v2.schema.json"
)
LEGACY_SCHEMA_PATH = SCHEMA_PATH.with_name("athena.shared_metrics.v1.schema.json")
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "athena.shared_metrics.v2"
    assert first_package["resource"] == {"athena_version": "test-version"}
...
def test_v2_package_preserves_pending_v1_model_counters(tmp_path):
    ...
    assert package["schema_version"] == "athena.shared_metrics.v2"
...
def test_v1_outbox_package_exports_unchanged_after_upgrade(tmp_path):
    ...
    payload = {
        "schema_version": "athena.shared_metrics.v1",
...
def test_package_schema_matches_the_model_call_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "athena.shared_metrics.v2"
...
def test_v1_package_schema_retains_the_legacy_model_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "athena.shared_metrics.v1"
