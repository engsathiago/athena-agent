...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_without_dist_waits_then_skips_fresh_build(self, tmp_path):
        ...
        import fcntl
        import threading
        from athena_cli.main import _build_web_ui as build
...
class TestBuildRecoversFromMissingToolchain:
    def test_reinstalls_and_retries_when_the_build_cannot_resolve_tsc(self, tmp_path):
        ...
        with patch("athena_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("athena_cli.main._run_with_idle_timeout", side_effect=[build_fail, build_ok]) as mock_build, \
             patch("athena_cli.main._web_ui_build_needed", return_value=True), \
             patch("athena_cli.main._write_web_ui_build_stamp"), \
             patch("athena_cli.main._time.sleep"):
            result = _build_web_ui(web_dir)
    ...
    def test_healthy_tree_builds_without_an_extra_install(self, tmp_path):
        ...
        with patch("athena_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("athena_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("athena_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_build, \
             patch("athena_cli.main._web_ui_build_needed", return_value=True), \
             patch("athena_cli.main._write_web_ui_build_stamp"):
            result = _build_web_ui(web_dir)
