...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    with patch("athena_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("athena_cli.main._desktop_build_needed", return_value=True), \
         patch("athena_cli.main._write_desktop_build_stamp"), \
         patch("athena_cli.main.subprocess.run", side_effect=[subprocess.CompletedProcess([], 0), launch_ok]), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("athena_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("athena_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("athena_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
