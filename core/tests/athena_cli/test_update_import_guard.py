...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    syntax_ok, _, _ = athena_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = athena_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert athena_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = athena_main._validate_critical_modules_import(tmp_path)
    assert ok is True
