...
from athena_cli import lifecycle, plugins as plugin_api
from athena_cli.observability import relay_runtime, relay_shared_metrics
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    monkeypatch.setenv(
        "ATHENA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
    monkeypatch.setattr(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("athena.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    packages = list(
        (athena_home / "telemetry" / "shared_metrics" / "outbox").glob("*.json")
    )
    assert len(packages) == 1
    package = json.loads(packages[0].read_text(encoding="utf-8"))
    assert package["metrics"][0]["name"] == "athena.model_route.count"
...
def test_relay_tool_request_rewrite_precedes_athena_authorization_boundary(
    tmp_path,
    monkeypatch,
):
