...
#
# Installs a real, earlier Athena the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `athena`.
#
...
# Keep sandbox state out of the default .athena-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".athena-sandbox-e2e-$ROUTE"
export ATHENA_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/athena/.athena/athena-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/athena-agent.git"
...
LOG_DIR="${ATHENA_E2E_LOG_DIR:-$(mktemp -d -t athena-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED athena accept FLAG on `athena update`?
#
...
# that silently rots. `athena update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "athena update --help 2>&1" | grep -qF -- "$flag"
}
...
require_athena_works() {
  local when="$1" out
  out="$(in_sandbox "athena --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "athena --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "athena runs $when"
}

# ── install the earlier Athena ─────────────────────────────────────────────
...
require_athena_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: athena update'
    ...
    # athena which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="athena update --yes"
    else
      update_cmd="athena update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "athena update failed ($update_cmd)"
    fi
    require_landed_on_target 'athena update'
    require_athena_works 'after athena update'
    ;;
  installer)
    ...
    require_athena_works 'after installer re-run'
    ;;
esac
