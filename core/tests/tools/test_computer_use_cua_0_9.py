...
def _browser_route(driver: _BrowserDriver, session_id: str = "athena-a"):
...
def test_exact_browser_binding_injects_athena_session_capability():
    driver = _BrowserDriver()
    route = _browser_route(driver, session_id="athena-owned-session")
    ...
    assert driver.calls == [
        (
            "get_browser_state",
            {"pid": 101, "window_id": 202, "session": "athena-owned-session"},
        )
    ]
...
def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="athena-a")
    second = _browser_route(driver, session_id="athena-b")
