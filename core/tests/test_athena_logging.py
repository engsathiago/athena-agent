...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, athena_home):
        ...
        agent_handlers = [
            h for h in athena_logging.rotating_file_handlers()
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in athena_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(athena_logging.sys, "platform", "win32"):
        ...
        with patch.object(athena_logging.sys, "platform", "linux"):
            assert not athena_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, athena_home):
        ...
        assert any(
            ...
            for h in athena_logging.rotating_file_handlers()
        )
