...
if not os.environ.get("ATHENA_HOME"):
...
def _capture_real_kanban_root() -> Path:
    ...
    if _PRE_SANDBOX_ATHENA_HOME:
        # ATHENA_HOME was genuinely set before the sandbox — honor it via the
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``athena_cli.kanban_db`` is *already imported* — a
