...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Athena' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Athena explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Athena and
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Athena document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`athena_cli.mcp_config._unwrap_exception_group` with
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Athena' logging (agent.log via athena_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `athena setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Athena process (kill -9, crash, force-quit)
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Athena processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_athena_home() / ".mcp-discovery.lock"
            )
...
# Native MCP tool-name prefix. Athena uses the ``mcp__<server>__<tool>``
