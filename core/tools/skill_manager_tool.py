...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch ATHENA_HOME and
    ...
    live profile-scoped ATHENA_HOME on every call.
    """
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`athena curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `athena curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `athena curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `athena sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `athena sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.athena/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `athena curator restore` promises the skill can be brought back. Route
...
def apply_skill_pending(payload: Dict[str, Any]) -> str:
    ...
    try:
        ...
        athena = athena_authorization_override(
            "persistent.write", "skills", subsystem="skills"
        )
    except Exception:
        athena = None
    if athena is not None and not athena["allowed"]:
        return tool_error(
            f"Persistent write denied by Athena policy: {athena.get('reason')}",
            success=False,
        )
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `athena curator
