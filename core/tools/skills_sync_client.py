...
409), and is driven by:
  ...
  * the ``athena sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``athena_cli/``) so the
...
is toggled via ``athena sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.athena/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``athena sync enable`` — it is
...
# namespace from athena-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ATHENA_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ATHENA_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Athena Cloud instance can be set
...
# precedence as base_url: the ATHENA_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   ATHENA_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ATHENA_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ATHENA_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``ATHENA_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Athena Cloud deployment sets to opt its instances into sync by
...
def sync_org_auto_propose() -> bool:
    ...
    ``ATHENA_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``athena sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ATHENA_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ATHENA_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``athena sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Athena Cloud deployment wants. Per the design notes, this default is
...
#
# Only agent-created + user-authored skills under ~/.athena/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.athena/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Athena Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``ATHENA_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.athena/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``athena sync device --name`` to set an explicit
    label."""
    ...
    # Athena Cloud (and any templated deployment) can seed the label
    # declaratively via ATHENA_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `athena sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("ATHENA_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.athena/skills/.sync_device_id, overwriting
...
# ~/.athena/skills/.sync_state as JSON.
#
...
# path under ~/.athena/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.athena/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Athena speaks "
...
def pull_skills(
    ...
    *,
    ...
    ~/.athena/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "athena skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ATHENA_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``athena sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.athena/skills/_org/
...
#
# `athena sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.athena/skills/_org/<org_id>/``.
