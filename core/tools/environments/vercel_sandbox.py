...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Athena'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``ATHENA_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Athena
...
def _snapshot_store_path() -> Path:
    return get_athena_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.athena"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_athena = (
            "/.athena"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.athena"
        )
        archive_member = remote_athena.lstrip("/")
