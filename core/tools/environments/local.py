...
def _athena_full_environment(target: str) -> bool:
    """Whether Athena authorizes passing the complete process environment."""
    try:
        from agent.athena_security_bridge import athena_authorization_override

        decision = athena_authorization_override("environment.inherit", target)
    except Exception:
        decision = None
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Athena was already launched from (for example ``athena-agent``
    while the process cwd is ``~/.athena/athena-agent``), passing it through
    unchanged makes the wrapper run ``cd athena-agent`` *inside* the project
    ...
    # Common recovery for config values like ``athena-agent`` when Athena was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./athena-agent``; use the current directory instead.
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Athena-managed inference credential — Claude subscription auth is not a
    # working Athena provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Athena venv path instead of the project's own
# ``.venv`` — silently clobbering the Athena environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Athena venv stays reachable via PATH (its bin dir is first), so stripping
...
def _is_athena_internal_secret(key: str) -> bool:
    """Return True for Athena-internal secrets injected under *dynamic* names.

    ``_ATHENA_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Athena-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`athena_subprocess_env` helper all call it, so the
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ATHENA_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    if _athena_full_environment("terminal"):
        sanitized = dict(base_env or {})
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_athena_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _ATHENA_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_athena_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_athena_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _ATHENA_PROVIDER_ENV_BLOCKLIST and not passthrough:
...
# legitimate child Athena spawns needs them, and they are the highest-value
...
# narrow subset of _ATHENA_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in athena_subprocess_env().
...
def athena_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_ATHENA_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Athena spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_ATHENA_PROVIDER_ENV_BLOCKLIST``
    ...
    """
    env = os.environ.copy()
    athena_full = _athena_full_environment("subprocess")

    # Tier 1 — always strip.
    if not athena_full:
        for key in _ALWAYS_STRIP_KEYS:
            env.pop(key, None)
    # Internal routing hints and Athena-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_athena_internal_secret`.
    if not athena_full:
        for key in list(env):
            if key.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
                env.pop(key, None)
    ...
    # copies os.environ, whose ATHENA_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's ATHENA_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`athena_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``ATHENA_HOME`` / subprocess ``HOME`` contract) and the
    Athena secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_athena_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``ATHENA_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Athena home override into ``ATHENA_HOME`` and apply the
      subprocess HOME contract (``athena_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ATHENA_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ATHENA_HOME override
...
def _find_bash() -> str:
    ...
    # system Git (or a stale ATHENA_GIT_BASH_PATH pointing at one) must not
    ...
    # ATHENA_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\athena\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ATHENA_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Athena-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Athena installs for
    itself, so on a machine where Athena provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$ATHENA_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$ATHENA_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_athena_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_athena_managed_node``, a first browser install).
    """
    try:
        from athena_constants import get_athena_home, iter_athena_node_dirs

        candidates = [*iter_athena_node_dirs(), get_athena_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Athena-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Athena
...
def _make_run_env(env: dict) -> dict:
    ...
    run_env = {}
    athena_full = _athena_full_environment("terminal")
    for k, v in merged.items():
        if k.startswith(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ATHENA_PROVIDER_ENV_FORCE_PREFIX):]
            if not athena_full and _is_athena_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif not athena_full and _is_athena_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if not athena_full and k in _ATHENA_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
