...
# the FIRST session's ATHENA_SESSION_ID into the snapshot makes every LATER
...
# ATHENA_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Athena' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is ATHENA_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ATHENA_SESSION_|ATHENA_UI_SESSION_ID|ATHENA_CRON_AUTO_DELIVER_|ATHENA_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ATHENA_SESSION_CHAT_NAME``) land in the
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        bootstrap = (
            f"umask 077\n"
            f"__athena_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__athena_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__athena_fns\" ] && declare -f $__athena_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # Restrict Athena metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__athena_snap_tmp=$(mktemp {_snap_tmp_template}) && "
