...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``ATHENA_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    container_ca = "/etc/ssl/certs/athena-egress-ca.crt"
    ...
    # through the proxy.  Keep the ATHENA_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from athena_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from athena_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_ATHENA_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_ATHENA_EGRESS_NODE_OPTIONS_APPEND", None,
        )
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # keys are filtered. Also strip Athena-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_athena_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_athena_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _ATHENA_PROVIDER_ENV_BLOCKLIST)
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # athena-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "athena egress disable",
