...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the Athena host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    Athena host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``athena
    ...
    be stale from ``athena setup``). Environment values for omitted terminal
    ...
    try:
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Athena cron for one-shot delayed work, or install an "
        ...
        if background:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `athena -z`, a "
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("ATHENA_INTERACTIVE"):
                    output += (
