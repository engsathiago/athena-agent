...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Athena provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `ATHENA_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Athena identity under athena_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["athena_identity"] = identity
