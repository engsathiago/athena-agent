...
* the driver session id is injected by the adapter, never accepted from the
  model;
* refs are usable only from the latest snapshot in this Athena session;
...
ordinary Athena tool approval never substitutes for protected consent.
"""
    ...
    def prepare(
        ...
        *,
        ...
        if profile_mode == "existing_profile":
            ...
            # explicit Athena YOLO owns a private unrestricted daemon.
            self.state.clear()
            return self._call(
                "browser_prepare",
                {
