...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and legacy upstream issue #67052.
    """
...
def _cua_no_overlay() -> bool:
    """True when Athena should pass ``--no-overlay`` to cua-driver.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Athena runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Athena process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Athena session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\athena-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of legacy upstream issue #47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Athena an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``ATHENA_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Athena Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `athena computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``ATHENA_CUA_DRIVER_CMD``) is never silently
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of legacy upstream issue #47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of legacy upstream issue #47072: per-tool
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `athena computer-use doctor` and check "
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    legacy upstream issue #47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of legacy upstream issue #47072: per-snapshot
    ...
    def capture(
        ...
        # Surface 3 of legacy upstream issue #47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of legacy upstream issue #47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See legacy upstream issue #67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # legacy upstream issue #47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Athena adapter, and uses cua-driver's
