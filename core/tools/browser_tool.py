...
# means a compromised transitive dependency could read every Athena secret
...
def _build_browser_env() -> dict:
    ...
    Strips Athena-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``athena_subprocess_env`` import is deferred to keep ``browser_tool``
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``athena doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``athena update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_athena_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_athena_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_athena_home() / "node")),
            ]
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/engsathiago/athena-agent:latest")
                else:
