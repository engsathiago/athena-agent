...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``athena-cli`` because those also contain
...
def _build_child_agent(
    ...
    # Blocked tools also live inside mixed platform bundles (athena-cli,
    # athena-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Athena/Qwen subagent onto the parent's Claude Messages wire (or the
...
def delegate_task(
    ...
    # ATHENA_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Athena-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Athena-Session-Id once the batch completes.
            ...
            # ATHENA_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `athena -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("ATHENA_SESSION_SOURCE", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ATHENA_SESSION_KEY is not in the CLI
...
def _load_config() -> dict:
    """Load delegation config from the active Athena config.

    Prefer the shared persistent loader because it follows the active
    ATHENA_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ATHENA_IGNORE_USER_CONFIG=1`` (``athena chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ATHENA_IGNORE_USER_CONFIG") == "1"
