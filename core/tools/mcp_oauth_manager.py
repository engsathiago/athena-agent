...
def _make_athena_provider_class() -> Optional[type]:
    ...
    class AthenaMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``athena mcp reauth``.
            ...
            back to ``athena mcp reauth``.
            """
            try:
                if self._athena_preregistered:
                    return
...
class MCPOAuthManager:
    ...
    def remove(
        ...
        *,
        ...
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, athena_home), None)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, athena_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        async with entry.lock:
            tokens_path = _get_token_dir(athena_home) / f"{_safe_filename(server_name)}.json"
            try:
