...
def _athena_gate(capability: str, target: str, **context) -> Optional[dict]:
    """Ask Athena first while leaving standalone Athena behaviour untouched."""
    try:
        from agent.athena_security_bridge import athena_approval_override

        return athena_approval_override(capability, target, **context)
    except Exception:
        return None
...
# os.environ["ATHENA_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ATHENA_INTERACTIVE.
_athena_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "athena_interactive",
    default=None,
)
...
def set_athena_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["ATHENA_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ATHENA_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ATHENA_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _athena_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ATHENA_INTERACTIVE")
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ATHENA_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("ATHENA_CRON_SESSION")
...
DANGEROUS_PATTERNS = [
    ...
    # `athena` and `gateway` (e.g. `athena -p ade gateway restart`) so a
    ...
    # a lifecycle command past the guard — same treatment as the `athena ...
    ...
    # the `athena gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.athena.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(athena|ai\.athena)\b', "stop/restart athena launchd service (kills running agents)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.athena/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ATHENA_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Athena home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\athena), so folding the user home
    # first would eat the prefix the Athena-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ATHENA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.athena``); the matched
...
def _rewrite_resolved_athena_home(command: str) -> str:
    ...
    /home/athena/.athena/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ATHENA_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.athena")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``athena send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"athena-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Athena YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Athena approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``ATHENA_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ATHENA_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "ATHENA_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ATHENA_INTERACTIVE or "
            "ATHENA_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}
...
def check_dangerous_command(command: str, env_type: str,
    ...
    """
    athena = _athena_gate(
        "terminal.execute", command, env_type=env_type,
    )
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ATHENA_INTERACTIVE``) fails CLOSED — a plugin-
    flagged action never runs ungated without a human.
    """
    athena = _athena_gate(
        "tool.execute", tool_name, tool_name=tool_name,
    )
    if athena is not None:
        return athena
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from athena_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    """
    athena = _athena_gate(
        "code.execute", code, env_type=env_type,
    )
    if athena is not None:
        return athena
