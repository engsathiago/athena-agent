...
def check_website_access(url: str, config_path: Optional[Path] = None) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        athena = athena_authorization_override("network.request", url)
    except Exception:
        athena = None
    if athena is not None:
        ...
        return {
            ...
            "rule": str(athena.get("rule_id") or "athena-policy"),
            "source": "athena",
            "message": f"Blocked by Athena policy: {athena.get('reason')}",
        }
