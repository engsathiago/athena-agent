...
# operational marker: without it, a sandbox script that spawns/imports Athena
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    try:
        ...
        athena = athena_authorization_override("environment.inherit", "execute_code")
    except Exception:
        athena = None
    if athena is not None and athena["allowed"]:
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    athena_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'athena_tools'", window
        )
        if m:
            missing = m.group(1)
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            ...
            f"ATHENA_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that calls Athena tools programmatically. "
