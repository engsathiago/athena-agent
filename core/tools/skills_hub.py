...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _athena_home() / "skills"
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official Athena skills but are not activated
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``athena_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _load_athena_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    athena_index_cache_file = _athena_index_cache_file()
    if athena_index_cache_file.exists():
        try:
            age = time.time() - athena_index_cache_file.stat().st_mtime
            if age < ATHENA_INDEX_TTL:
                return json.loads(athena_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass

    if not ATHENA_INDEX_URL:
        return _load_stale_index_cache()

    # Fetch from an explicitly configured Athena index.
    #
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Athena index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        athena_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
