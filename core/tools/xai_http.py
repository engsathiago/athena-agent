...
def has_xai_credentials() -> bool:
    ...
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``athena auth add xai-oauth``
...
def athena_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Athena Agent,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Athena config section as a dict, or empty."""
    try:
    ...
    *,
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("ATHENA_XAI_BASE_URL")
