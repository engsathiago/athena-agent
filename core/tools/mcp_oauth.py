...
def _get_token_dir(athena_home: str | Path | None = None) -> Path:
    ...
    base = Path(athena_home) if athena_home is not None else Path(get_athena_home())
...
def _cached_redirect_port(storage: "AthenaTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Athena restarts and chooses a
...
def _cached_redirect_uri(storage: "AthenaTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``athena mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `athena mcp login <server>` interactively to (re)authorize, "
...
class AthenaTokenStorage:
    ...
    def _tokens_path(self) -> Path:
        return _get_token_dir(self._athena_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._athena_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._athena_home) / f"{self._server_name}.meta.json"
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See the OAuth-over-SSH guide included with the Athena source.\n",
                file=sys.stderr,
            )
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = AthenaTokenStorage(server_name, athena_home=athena_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "AthenaTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Athena Agent" / "Athena" / "Cursor" / "VS Code" / … → 403
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Athena auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Athena is running.
    """
    msg = str(exc)
    lowered = msg.lower()
