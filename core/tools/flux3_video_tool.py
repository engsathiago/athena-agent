...
_SIGN_IN_MESSAGE = (
    ...
    "Ask the user to run `athena model` and sign in to Nous, then retry."
)
...
def _default_directory():
    ...
    is not: an operator running ATHENA_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_athena_dir("cache/videos", "video_cache")
        except Exception:
