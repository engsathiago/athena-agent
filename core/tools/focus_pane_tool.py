#!/usr/bin/env python3
"""Reveal/focus a pane in the Athena desktop GUI.

Gated on ``ATHENA_DESKTOP`` (like the other GUI affordances). Emits
...
def focus_pane_tool(pane: str) -> str:
    ...
    if not ok:
        return tool_error("Pane focus is only available in the Athena desktop app.")
...
def check_focus_pane_requirements() -> bool:
    ...
    return env_var_enabled("ATHENA_DESKTOP")


FOCUS_PANE_SCHEMA = {
    "name": "focus_pane",
    "description": (
        "Reveal and focus a pane in the Athena desktop app when the user asks to "
