...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id (X-Athena-Session-Id) of the ORIGINATING
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``ATHENA_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``ATHENA_SESSION_CHAT_ID`` binding survives child
    construction: ``_bind_api_server_session`` binds ``chat_id`` to the raw
    ``X-Athena-Session-Id``, and its only writer is ``set_session_vars`` —
    ...
    try:
        ...
        if get_session_env("ATHENA_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("ATHENA_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_athena_home() under the right profile.
