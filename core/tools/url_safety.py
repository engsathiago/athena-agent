...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. Athena-owned direct httpx request paths should use
...
def _athena_network_decision(url: str) -> Optional[bool]:
    """Return Athena's network override, or None when Athena remains owner."""
    try:
        from agent.athena_security_bridge import athena_authorization_override

        decision = athena_authorization_override("network.request", url)
    except Exception:
        decision = None
    if decision is None:
        return None
...
def is_always_blocked_url(url: str) -> bool:
    ...
    """
    athena = _athena_network_decision(url)
...
def _resolved_http_connect_ips(host: str, port: int, scheme: str) -> list[str]:
    ...
    athena = _athena_network_decision(f"{scheme}://{hostname}:{port}")

    if athena is False:
        raise SSRFConnectionBlocked(f"Blocked by Athena network policy: {hostname}")
    if athena is None and hostname in _BLOCKED_HOSTNAMES:
    ...
    for _family, _, _, _, sockaddr in addr_info:
        ...
        if athena is None and (
            ip in _ALWAYS_BLOCKED_IPS or any(ip in net for net in _ALWAYS_BLOCKED_NETWORKS)
        ):
            raise SSRFConnectionBlocked(
                f"Blocked request to cloud metadata address during connect: {hostname} -> {ip_str}"
            )

        if athena is None and not allow_all_private and not allow_private_ip and _is_blocked_ip(ip):
            raise SSRFConnectionBlocked(
