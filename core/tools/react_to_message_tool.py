#!/usr/bin/env python3
"""Let the agent react to a message with an emoji in the Athena desktop app.
...
Gated on ``ATHENA_DESKTOP`` (like the other GUI affordances) so it costs nothing
...
def react_to_message_tool(emoji: str, message_row_id=None, messages_back=None) -> str:
    ...
    session_key = get_session_env("ATHENA_SESSION_KEY", "") or get_session_env(
        "ATHENA_SESSION_ID", ""
    )
