...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Athena venv at
    # ~/.athena/profiles/<name>/athena-agent/venv/), `pip install` on the
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.athena/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``AthenaCLI._voice_start_recording``). A value <= 0
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Athena TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"athena-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("ATHENA_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    ATHENA_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
