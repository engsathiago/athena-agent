...
# Base URL now resolved via athena_cli.models.deepinfra_base_url (shared).
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``athena tools``); otherwise the historical Edge backend remains active.
    """
...
# file, so retired models disappear from athena the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``athena_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `athena setup` to configure, "
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Athena client context following Gemini's partner
