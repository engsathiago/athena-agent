...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where athena previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Athena.
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    try:
        ...
        decision = athena_authorization_override(
            "file.write", filepath, session_id=task_id
        )
    except Exception:
        decision = None
    if decision is not None:
        if decision["allowed"]:
            return None
        return f"Refusing write by Athena policy: {decision.get('reason')}"
