...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ATHENA_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/athena`` read-only), setting
  ``ATHENA_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Athena, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (ATHENA_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (athena trace upload / /upload-trace).
    #
    ...
    # presence, the `athena update` lazy-refresh pass re-asserts THIS pin on
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ATHENA_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``athena update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Athena under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Athena installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Athena
    ...
    # deployment sets ATHENA_MANAGED=true *and* ATHENA_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Athena cannot install "
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ATHENA_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ATHENA_LAZY_INSTALL_TARGET``, installs are redirected to the writable
