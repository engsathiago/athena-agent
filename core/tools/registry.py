...
def _athena_plugin_override(tool_name: str) -> bool:
    try:
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (athena_constants itself is stdlib-only, so no cycle).
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Athena-home override for every
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("athena_plugins."):
            return mod
        return None
        ...
        with self._lock:
            ...
            if existing and existing.toolset != toolset:
                if override:
                    ...
                    if (
                        _owner is not None
                        and not self._plugin_override_policy.get(_owner, False)
                        and not _athena_plugin_override(name)
                    ):
                        logger.error(
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``athena_plugins.{name}``), not the exact module string.
                # A handler defined in ``athena_plugins.pkg.handlers`` is
                # still owned by the ``athena_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("athena_plugins.")
                    and not same_plugin
