...
# uses 30s) — comfortably below the 1800s default ATHENA_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (ATHENA_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with ATHENA_CRON_TIMEOUT=0
...
def _scan_cron_prompt(prompt: str) -> str:
    ...
    try:
        ...
        athena = athena_authorization_override("cron.schedule", prompt)
    except Exception:
        athena = None
    if athena is not None:
        return "" if athena["allowed"] else f"Blocked by Athena policy: {athena.get('reason')}"
...
def _scan_cron_skill_assembled(assembled: str) -> tuple[str, str]:
    ...
    try:
        ...
        athena = athena_authorization_override("cron.execute", assembled)
    except Exception:
        athena = None
    if athena is not None:
        if athena["allowed"]:
            return assembled, ""
        return assembled, f"Blocked by Athena policy: {athena.get('reason')}"
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            # send_message, which passes ATHENA_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``ATHENA_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _validate_cron_base_url(
    ...
    try:
        ...
        athena = athena_authorization_override("network.request", bu)
    except Exception:
        athena = None
    if athena is not None:
        return None if athena["allowed"] else f"Blocked by Athena policy: {athena.get('reason')}"
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        athena = athena_authorization_override("cron.script", script)
    except Exception:
        athena = None
    if athena is not None:
        return None if athena["allowed"] else f"Blocked by Athena policy: {athena.get('reason')}"
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `athena cron create/edit --model`, or hand-edited jobs). The agent
