...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``athena auth add <provider_id>``).
...
# DeepInfra STT base URL now resolved via athena_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ATHENA_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Athena's remote upload cap."""
...
def _command_stt_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Athena secrets by
...
def _run_command_stt(
    ...
    Child env is scrubbed of Athena secrets (salvage of #56332) while still
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="athena-local-stt-") as output_dir:
            ...
            # Scrub Athena secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import athena_subprocess_env

            child_env = athena_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``ATHENA_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``athena_cli.models`` helpers so every DeepInfra surface resolves the
