...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    inherited ATHENA_KANBAN_* env vars are not proof of dispatcher ownership.
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``ATHENA_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``ATHENA_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("ATHENA_KANBAN_TASK")
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    # athena_cli/goals.py. Unpacking fewer raises ValueError,
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "athena-kanban/attach"},
            timeout=30,
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; Athena downloads it (with the shared size cap)
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # Prefer the request-scoped api_server origin binding: ATHENA_SESSION_ID
    ...
    session_id = (
        ...
        or os.environ.get("ATHENA_SESSION_ID")
    )
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``ATHENA_SESSION_PLATFORM``,
      ``ATHENA_SESSION_CHAT_ID``, and ``ATHENA_SESSION_CHAT_TYPE`` are set in
    ...
    try:
        ...
        chat_type = get_session_env("ATHENA_SESSION_CHAT_TYPE", "") or None
        message_id = get_session_env("ATHENA_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = (
            get_session_env("ATHENA_SESSION_PROFILE", "")
            or os.environ.get("ATHENA_PROFILE")
        )
