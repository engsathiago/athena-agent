...
wake word, Athena opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey athena" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey athena" can't
...
# Bundled "hey athena" model (tools/wakewords/) — the default, so the wake word
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Athena backend "
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Default (or explicit "hey_athena") → the bundled model; a built-in name
...
# ATHENA_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey athena", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey athena" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey athena").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="athena-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `athena tools` "
