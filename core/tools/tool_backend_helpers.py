...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``athena auth add
    ...
    2. The environment / ``~/.athena/.env``. Under a multiplexed gateway turn
       ...
       ``athena_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``athena auth
    ...
    when omitted, ``athena_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `athena auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``athena auth add openai-api``) via
