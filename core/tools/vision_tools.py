...
# ``agent.auxiliary_client`` pulls credential_pool → athena_cli.auth → httpx
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: ATHENA_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("ATHENA_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
