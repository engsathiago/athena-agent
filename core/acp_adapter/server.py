...
from tools.approval import (
    reset_athena_interactive_context,
    set_athena_interactive_context,
)
...
# already uses (`athena_cli/moa_cmd.py`). This bounds each provider's row, not
...
class AthenaACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Athena inventory is also used by ``athena model``, the TUI,
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.athena`` (ACP's extensibility
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # tools.approval (set_athena_interactive_context) rather than
        # os.environ["ATHENA_INTERACTIVE"], so concurrent executor workers can't
        ...
        def _run_agent() -> dict:
            ...
            try:
                ...
                # Without it the prompt advertises the global Athena workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.athena/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global ATHENA_CRON_SESSION (#37968).
            ...
            interactive_token = set_athena_interactive_context(True)
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_athena_interactive_context(interactive_token)
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Athena install tree into the session's
