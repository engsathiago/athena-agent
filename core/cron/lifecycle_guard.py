...
``athena gateway restart`` (or ``launchctl kickstart ai.athena.gateway``
or ``systemctl restart athena-gateway``).  When the cron fires, the
...
``athena cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``athena gateway``, ``launchctl ... athena-gateway``,
``systemctl ... athena-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_ATHENA_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``athena gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `athena gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:athena\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a athena-gateway label. macOS launchd
    # labels look like `ai.athena.gateway` / `athena-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated athena services (e.g.
    # `launchctl unload ai.athena.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.athena.gateway-<suffix> --
    ...
    # Branch D: pkill / kill targeting the athena gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.athena.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.athena.svc-reload-tmp``)
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".athena"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<ATHENA_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.athena/scripts/restart.sh``) but is passed as the bare name
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `athena gateway restart` embedded in a .py script is still
