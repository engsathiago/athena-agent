...
# store under its own ATHENA_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same ATHENA_HOME — so a job authored in profile
# `coder` lives in `~/.athena/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_athena_home()` (the active profile home), NOT `get_default_athena_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever ATHENA_HOME the
...
# `_get_athena_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_athena_home()
       (context-local override, then the ATHENA_HOME env var) — so a test
       or embedder that re-points ATHENA_HOME after this module was
...
# the cron inactivity timeout is disabled (ATHENA_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. ATHENA_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``ATHENA_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec athena athena cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Athena timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `athena_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                athena_tz = _athena_now().tzinfo
                dt = dt.replace(tzinfo=athena_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``athena cron status``) never sees a torn/truncated
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``athena cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``athena cron
...
def create_job(
    ...
    # covered, not just `athena cron create`.
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from ATHENA_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one ATHENA_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from ATHENA_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`athena_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``athena_cli.backup._prune_quick_snapshots``:
