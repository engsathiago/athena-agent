...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global ATHENA_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `athena cron
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `athena cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_athena_home_override()`` + ``use_cron_store()``
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `athena cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
