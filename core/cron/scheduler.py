...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``athena update`` / ``athena gateway stop`` / systemd restart, or an
...
def _get_athena_home() -> Path:
    ...
    profile-scoped gateway, so resolving the active ATHENA_HOME at call time
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Athena — {task_name}"
    try:
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from ATHENA_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("ATHENA_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ATHENA_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    _ctx_tokens = set_session_vars(
        ...
        # ATHENA_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ ATHENA_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_session_var = _VAR_MAP["ATHENA_CRON_SESSION"]
    ...
    try:
        ...
        # load_athena_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this ATHENA_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_athena_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from athena_cli.env_loader import (
            load_athena_dotenv,
        ...
        # cron-fleet default) > ATHENA_MODEL env > config.yaml ``model:``
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_athena_home())
        )
