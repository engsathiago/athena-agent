...
from athena_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``athena -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.athena`` even when a test fixture later
    redirects ``ATHENA_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_athena_home()`` fresh so a runtime
       ``ATHENA_HOME`` redirect takes effect regardless of import order.
    """
...
def _wal_reset_repair_hint() -> str:
    ...
    ``athena update`` can actually do for this install (#75153).
    """
    try:
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Athena"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Athena-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `athena doctor`. This warning fires once per "
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with athena_cli.config.
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``athena_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Athena home tree (``get_athena_home()``) — the safe repair scope:
      Athena owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``athena_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_athena_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Athena needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (athena_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_athena_state.py:600-632)
...
# (``~/.athena/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``athena sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    return get_athena_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ATHENA_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship athena_state without athena_cli). A
    ...
    try:
        ...
        logger.debug(
            "athena_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch athena_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``athena_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `athena snapshot list` / "
                "`athena snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Athena
    ...
    # `athena update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `athena snapshot list` / "
                    f"`athena snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`athena db optimize`),
    ...
    #
    # `optimize_fts_storage()` (the `athena sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`athena sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``athena --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: athena_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``athena -c``/``--resume`` so the "last" session is the last one in
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ATHENA_SESSION_SOURCE``, so their runs
