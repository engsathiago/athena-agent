...
# from shadowing Athena's own top-level modules.  ``athena_bootstrap`` lives at
...
# shared owner in athena_cli.mcp_startup — the wait/in-flight/join helpers
...
# athena_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # → athena_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's ATHENA_HOME first), so a launch profile with no
    ...
    try:
        from athena_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``athena --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``athena dashboard`` spawn theirs via
    ``athena_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``athena_cli.mcp_startup`` thread used by the
    ...
    try:
        from athena_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's ATHENA_HOME override — the shared owner in
    ``athena_cli.mcp_startup`` captures the caller's context-local override
