...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ATHENA_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Athena agent → needs provider credentials.
        ...
        # gateway's launch ATHENA_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ATHENA_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            athena_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"ATHENA_HOME": str(profile_home)} if profile_home else None,
        )
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s ATHENA_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_athena_home``).
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``athena dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ATHENA_COMPUTE_HOST_CHILD") == "1"
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``athena`` / ``athena --tui`` / CLI) is started from a
    ...
    try:
        ...
        from athena_state import is_disk_full_error
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``athena_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``athena_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ATHENA_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ATHENA_SESSION_ID even though agent_init set it via
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``athena config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Athena activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ATHENA_DESKTOP=1`` and ``ATHENA_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ATHENA_DESKTOP_TERMINAL=1`` → "tui"
        (``athena --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``athena --tui``.)
    """
    if is_truthy_value(os.environ.get("ATHENA_DESKTOP")) and not is_truthy_value(
        os.environ.get("ATHENA_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ATHENA_MODEL /
    ATHENA_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`athena --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ATHENA_MODEL /
    # ATHENA_INFERENCE_MODEL, which `athena --tui -m <model>` sets as a
...
def _load_approval_mode() -> str:
    ...
    canonical ``athena_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`athena_constants.resolve_reasoning_config` (per-model override >
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _ATHENA_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off athena-cli's tool universe — can't
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `athena --tui -m`
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ATHENA_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ATHENA_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    ...
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_athena_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Athena has to support both the legacy 8-row petdex atlas and the current
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ATHENA_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + athena_cli/nous_billing.py) — same as /topup.
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ATHENA_SESSION_KEY`` (see
    ...
    # DB when ATHENA_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ATHENA_UI_SESSION_ID`` as
    ...
    *,
    ...
    def run():
        ...
        try:
            ...
            # Run while any profile-specific ATHENA_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ATHENA_HOME
    override, so ``get_athena_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _athena_home
    return base / "images"
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under ATHENA_HOME (~/.athena by default) — config/sessions/skills,
    ...
    return real == home or real == athena_home or real.startswith(athena_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ATHENA_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    return real == home or real == athena_home
...
def _discover_repos_payload(
    ...
    repos even with zero athena sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (athena home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Athena sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Athena
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Athena" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``athena_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via athena_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import athena_cli.voice  # noqa: F401
