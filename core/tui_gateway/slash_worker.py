...
# from shadowing Athena's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``athena_bootstrap``
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before AthenaCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``athena serve``
