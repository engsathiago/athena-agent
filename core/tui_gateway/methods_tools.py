...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m athena` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m athena` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("ATHENA_API_KEY", "") or cfg.get("api_key", "")
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``athena journey`` CLI.
    """
    try:
