...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(athena_home / "cache" / "bws_cache.enc.json"),
            str(athena_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved ATHENA_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("ATHENA_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    try:
        ...
        decision = athena_authorization_override("file.write", path)
    except Exception:
        decision = None
    if decision is not None:
        if decision["allowed"]:
            return None
        return "athena:" + str(decision.get("reason") or "owner policy denied write")
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if isinstance(denial, str) and denial.startswith("athena:"):
        return f"{verb} denied by Athena policy: {denial.split(':', 1)[1]}"
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    try:
        ...
        decision = athena_authorization_override("file.read", path)
    except Exception:
        decision = None
    if decision is not None:
        if decision["allowed"]:
            return None
        return f"Access denied by Athena policy: {decision.get('reason')}"
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Athena read (see
