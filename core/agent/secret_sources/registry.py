...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``athena`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's ATHENA_HOME is ``~/.athena/profiles/<name>``; the
    default profile (``~/.athena``) returns "".
    """
