...
cache problem must never block Athena startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Athena home used for cache paths.

    ``home_path`` is whatever ``load_athena_dotenv()`` already resolved;
    falling back to ``$ATHENA_HOME`` / ``~/.athena`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<athena_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ATHENA_HOME is ever made traversable.
            try:
