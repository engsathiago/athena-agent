...
``CommandSecretsProvider`` (athena-desktop ``src/main/secrets/commandProvider.ts``)
...
* The requested key is passed to the child ONLY via the ``ATHENA_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``ATHENA_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# athena_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``ATHENA_SECRET_KEY`` — never interpolated
