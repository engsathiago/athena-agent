...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$ATHENA_HOME` / `~/.athena` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under athena_home/cache/."""
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Athena
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`athena secrets bitwarden token`) so the
