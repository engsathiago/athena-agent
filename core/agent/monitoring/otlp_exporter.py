...
configured under ``monitoring.export.otlp``. Lets an operator stream Athena
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install athena-agent[otlp]``), imported lazily so the
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    for ev in batch:
        try:
            name = f"athena.{ev.get('event', 'event')}"
