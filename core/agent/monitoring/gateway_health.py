...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("athena.gateway.up", 1 if gateway_running else 0, base),
        _metric("athena.gateway.active_agents", active_agents, base),
        _metric("athena.gateway.busy", 1 if busy else 0, base),
        _metric("athena.gateway.drainable", 1 if drainable else 0, base),
        _metric("athena.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("athena.gateway.state", 1, base, **{"athena.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"athena.platform": str(platform), "athena.platform.state": state},
        ))
        metrics.append(_metric(
            "athena.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"athena.platform": str(platform), "athena.platform.state": state, "athena.error_code": error_code},
        ))
        if is_degraded:
