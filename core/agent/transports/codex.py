...
# Wire-name used when Athena keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Athena's configured provider
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Athena web providers.
        #
        ...
        #    keep Athena dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``athena_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Athena dispatches
