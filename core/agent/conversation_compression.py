...
# Mirror athena_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``athena_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["ATHENA_SESSION_ID"] = child_session_id
    try:
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Athena's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Athena' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|athena|off).
    # The memory-provider context handoff below is intentionally Athena-only:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the ATHENA_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("ATHENA_SESSION_SOURCE", "cli"),
                        model=agent.model,
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Athena' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Athena' transcript
    unchanged.
    """
    auto_mode = str(
