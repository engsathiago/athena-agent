...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Athena state.

    The app-server owns the compacted thread context, so Athena should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Athena deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Athena UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Athena tool loop. Without this bridge gateway
...
# Codex item types that map to a Athena tool_call in the projector (and
...
# Internal MCP server that wraps Athena' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# athena-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.athena-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as Athena tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "athena-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Athena tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Athena' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Athena' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # explicitly opted out of Athena approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / ATHENA_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Athena UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Athena' gateway UI callbacks
