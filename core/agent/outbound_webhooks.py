...
(``gateway/platforms/webhook.py``): inbound wakes Athena when the world
changes; outbound tells the world when Athena does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Athena-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``ATHENA_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.athena/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/athena-events
          ...
          secret_env: ATHENA_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `athena hooks list`
...
Headers::
    ...
    User-Agent:              Athena-Agent-Outbound-Webhook
    X-Athena-Event:          <hook event name>
    X-Athena-Delivery:       <delivery_id>
    X-Athena-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Athena-Delivery`` header so
