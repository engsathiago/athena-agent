...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in athena_state.py — kept local
# to avoid importing athena_state at module load time (its module-level
# DEFAULT_DB_PATH = get_athena_home() / "state.db" breaks tests that
# monkeypatch get_athena_home to return a str).
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.athena/prefill.json incident, 20/20 blocked with
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Athena' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
def run_conversation(
    ...
    # Adopt any ~/.athena/.env credential/base-url edits made since the last
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from athena_cli.lifecycle import (
                        has_hook,
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Athena may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See athena-lcm engine
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.athena/config.yaml` to extend the per-call "
                            f"timeout. (Athena's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Athena mid-turn, resume logic still sees the
                ...
                # timeout (ATHENA_AGENT_TIMEOUT, default 1800s) and the
