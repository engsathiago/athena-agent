"""Upload a Athena session transcript to Hugging Face as an agent trace.

Athena stores sessions in its own SQLite store (``athena_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``athena trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Athena' secret redactor (``force=True``) unless the
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Athena' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Athena conversation messages as Claude Code JSONL text.
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/athena-traces`` dataset. Returns a
