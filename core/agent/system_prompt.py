...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ATHENA_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ATHENA_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_athena_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            ...
            f"and writes {get_athena_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_athena_home()}/skills/, {get_athena_home()}/plugins/, "
            f"{get_athena_home()}/cron/, {get_athena_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Athena). Every other surface (desktop chat panel,
