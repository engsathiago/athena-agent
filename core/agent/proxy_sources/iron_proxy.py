...
* The ``iron-proxy`` binary is auto-installed into ``<athena_home>/bin/iron-proxy``
  on first use.  Athena pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<athena_home>/proxy/ca.{crt,key}`` is generated on
  first ``athena egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<athena_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``athena egress start``), pidfile
  at ``<athena_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<athena_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``athena egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<athena_home>/proxy/management.token`` (0600), and injected into the
...
# credential (Athena' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.athena/proxy/`` just to
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<athena_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``athena proxy install`` surface lets them
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``athena egress
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<athena_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``ATHENA_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``athena egress reload`` reads the same file to authenticate.
    """
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `athena egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `athena egress setup` and use "
            "`athena egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `athena egress setup`, "
            "then `athena egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``athena egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `athena egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<athena_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Athena dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``athena egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``athena egress setup`` should not invalidate the tokens
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_ATHENA_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `athena egress install`."
        )
    ...
    # close-on-success path, every restart leaked one fd in the Athena
    # process.
    #
    ...
    # ``athena egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``athena egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `athena egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`athena egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `athena secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
