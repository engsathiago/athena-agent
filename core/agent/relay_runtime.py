...
class RelayRuntime:
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Athena session, if active."""
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Athena session identified by ``event``."""
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Athena Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Athena Relay runtime operation failed", exc_info=True)
            return None
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Athena Relay runtime initialization failed", exc_info=True
                )
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Athena Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"athena.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Athena Relay conversation initialization failed",
                    exc_info=True,
                )
        ...
        *,
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Athena turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Athena turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning("Athena Relay turn initialization failed", exc_info=True)
                ...
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        try:
                            ...
                            logger.warning(
                                "Athena Relay turn finalization failed", exc_info=True
                            )
            finally:
                try:
                    ...
                    logger.warning(
                        "Athena Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            try:
                ...
                logger.warning(
                    "Athena Relay logical LLM finalization failed",
                    exc_info=True,
                )
                break
