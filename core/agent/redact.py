...
def redact_sensitive_text(
    ...
    *,
    ...
    try:
        ...
        decision = athena_authorization_override("secret.reveal", text)
    except Exception:
        decision = None
    if decision is not None:
        if decision["allowed"]:
            return text
        # A controlled Athena deny means secret revelation is denied, so force
        # redaction even if a compatibility environment flag disabled Athena'
        # default redactor earlier in this long-running process.
        force = True
