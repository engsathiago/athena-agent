...
# un-throttled log fires on *every* selection. On Windows several Athena
...
# readiness handshake ("Timed out connecting to Athena backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``athena_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Athena
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ATHENA_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ATHENA_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ATHENA_XAI_REFRESH_TIMEOUT_SECONDS"
        )
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        try:
            ...
            # (athena auth remove copilot gh_cli) must not pay either on
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_athena_dotenv() -> apply_onepassword_secrets()).  The raw
