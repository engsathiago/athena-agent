...
def _ensure_reference_path_allowed(path: Path) -> None:
    try:
        ...
        athena = athena_authorization_override("file.attach", str(path))
    except Exception:
        athena = None
    if athena is not None:
        if athena["allowed"]:
            return
        raise ValueError(f"path denied by Athena policy: {athena.get('reason')}")
    ...
    # message text into reference expansion, so `@file:~/.athena/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Athena path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
