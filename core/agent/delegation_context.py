...
The parent Athena process may itself be a Kanban dispatcher worker with
ATHENA_KANBAN_* variables in process env. delegate_task children run inside the
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``ATHENA_KANBAN_*`` vars while losing the ContextVar in
