...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ATHENA_CA_BUNDLE`` /
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Athena owns auxiliary retry + provider/model fallback policy (the
    ...
    # Athena sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Athena control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="athena-protected-aux-provider",
        daemon=True,
    ).start()
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Athena plumbing (_reasoning_config
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=athena_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``athena_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._athena_fallback_destination = _fallback_destination_from_entry(
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = athena_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Athena owns
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(athena_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Athena reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Athena reasoning config for direct model calls
