...
row taxonomy is inferred from the atlas shape so Athena can render both legacy
...
# rows (1536x1872), while older Athena/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Athena' activity state names. They are not always identical to the
...
# Legacy Athena/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Athena activity names -> accepted row-name aliases in descending
