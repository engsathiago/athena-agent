...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("ATHENA_STREAM_STALE_TIMEOUT", 180.0)
...
def direct_api_call(agent, api_kwargs: dict):
    ...
    timeout (provider ``request_timeout_seconds`` / ``ATHENA_API_TIMEOUT``) bounds
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # limit. Tunable via ATHENA_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("ATHENA_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    if _ttfb_timeout <= 0:
        ...
        _ttfb_disable_above = _env_float("ATHENA_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        ...
        if (
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set ATHENA_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # substitute it before dropping the key (Athena bookkeeping,
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    if agent.api_mode == "bedrock_converse":
        ...
        while t.is_alive():
            ...
            if _stale_elapsed > _bedrock_stale_timeout:
                ...
                # crosses ATHENA_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Athena. Preserve only
        ...
        if agent.provider == "moa":
            # Athena interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Athena preserves
    ...
    # unless the user explicitly set ATHENA_STREAM_STALE_TIMEOUT; override the
    # local ceiling with ATHENA_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``ATHENA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("ATHENA_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
