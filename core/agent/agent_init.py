...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ATHENA_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ATHENA_HOME`` just means the notice
...
def init_agent(
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from athena_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = athena_xai_default_headers()
    ...
    try:
        from athena_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `athena --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Athena behavior) or left to LM Studio's
    ...
    if codex_app_server_auto_compaction not in {"native", "athena", "off"}:
        _ra().logger.warning(
