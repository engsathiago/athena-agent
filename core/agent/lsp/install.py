...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `athena lsp status` reports the host's presence.
...
def _install_npm(
    ...
    """
    # Managed npm first: $ATHENA_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Athena installed and
