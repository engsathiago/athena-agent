...
- **skills** → the skill name (e.g. ``"debugging-athena-desktop"``)
...
shared by the CLI (``athena journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``athena curator restore``); deleting a memory rewrites its
