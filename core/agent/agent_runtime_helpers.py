...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Athena-Session-Id header), and an
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to athena's outer conversation loop,
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: athena' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Athena' keepalive client (#10324 / ``_build_keepalive_http_client``) and
