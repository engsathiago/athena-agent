...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``ATHENA_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``ATHENA_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``ATHENA_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``ATHENA_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``ATHENA_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
