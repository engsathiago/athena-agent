...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Athena process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("ATHENA_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid ATHENA_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Athena process before the normal turn-end persistence path runs.
