...
Defenses 1 and 2 (the ``_ATHENA_GATEWAY`` guard on ``athena gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.athena.gateway")``,
...
State lives in ``<ATHENA_HOME>/gateway/restart_loop.json`` so it is
