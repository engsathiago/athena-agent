...
* Stateless request/response adapters (the API server,
  ...
  ``X-Athena-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Athena-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Athena-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Athena-Session-Id`` is 403-gated on
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "athena-agent"),
