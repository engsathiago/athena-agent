...
Athena' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``ATHENA_HOME`` either by using
conventional paths::

    ~/.athena/status_phrases.yaml
    ~/.athena/status_phrases/*.yaml
...
# These are Athena UI surfaces, not app/vendor/domain buckets.  Keep this
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None, platform_key: str | None = None) -> dict[str, list[str]]:
    ...
    _merge_phrase_config(catalog, display.get("generic_status_phrases"), base_dir=athena_home)
    _merge_phrase_config(catalog, display.get("status_phrases"), base_dir=athena_home)
    ...
    if platform_key and isinstance(platforms, Mapping):
        ...
        if isinstance(platform_display, Mapping):
            _merge_phrase_config(catalog, platform_display.get("generic_status_phrases"), base_dir=athena_home)
            _merge_phrase_config(catalog, platform_display.get("status_phrases"), base_dir=athena_home)
    return catalog
