...
class GatewayAuthorizationMixin:
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        try:
            ...
            identity = source.user_id or source.chat_id or "anonymous"
            athena = athena_authorization_override(
                "gateway.receive",
                f"{platform_value}:{identity}",
                profile=getattr(source, "profile", None) or "",
                session_id=getattr(source, "session_id", None) or "",
        ...
        # (athena gateway pairing approve / the authenticated dashboard) — an
