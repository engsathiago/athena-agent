...
_SESSION_CHAT_TYPE: ContextVar = ContextVar("ATHENA_SESSION_CHAT_TYPE", default=_UNSET)
...
# intentionally separate from ATHENA_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("ATHENA_UI_SESSION_ID", default=_UNSET)
...
_SESSION_PROFILE: ContextVar = ContextVar("ATHENA_SESSION_PROFILE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("ATHENA_CRON_SESSION", default=_UNSET)
...
_VAR_MAP = {
    ...
    "ATHENA_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    ...
    "ATHENA_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "ATHENA_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "ATHENA_SESSION_PROFILE": _SESSION_PROFILE,
    "ATHENA_CRON_SESSION": _CRON_SESSION,
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``ATHENA_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See AthenaPRDelegationSessionContext task.
    try:
...
def set_session_vars(
    ...
    ``os.environ["ATHENA_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``ATHENA_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``ATHENA_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
# value (``telegram``) to ATHENA_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind ATHENA_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# ATHENA_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``ATHENA_PLATFORM``, then the session platform, then the session
...
def declare_stateless_channel() -> None:
    ...
    See legacy upstream issue #53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``athena -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``ATHENA_KANBAN_TASK``), which are one-shot
