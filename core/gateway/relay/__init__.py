...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``athena gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**Athena Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``athena gateway enroll`` CLI. Two modes, in precedence order:
