...
# dashboard's pre-write mutation validation (athena_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Athena is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # Parse profile routes (validated by gateway.profile_routing). Athena
        # contributes its owner-managed peer bindings without rewriting the
        # upstream Athena config file.
        from gateway.profile_routing import parse_profile_routes
        raw_profile_routes = list(data.get("profile_routes") or [])
        if os.environ.get("ATHENA_RUNTIME", "").strip().lower() in {"1", "true", "yes", "on"}:
            try:
                from athena.bindings import routing_entries

                athena_routes = routing_entries()
                raw_profile_routes.extend(athena_routes)
                if athena_routes:
                    multiplex_profiles = True
            except Exception:
                pass
        profile_routes = parse_profile_routes(raw_profile_routes)
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # (e.g. via `athena config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `athena config set gateway.multiplex_profiles true`
