...
# ``ATHENA_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``ATHENA_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `athena tools` (opt-in, default
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Athena session "
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.athena/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.athena/state.db "
                "and are shown by `athena sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from athena_state import CompressionSessionClosedError
