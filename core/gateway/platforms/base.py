...
# as MPEG-2 Layer II are audio to Athena but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``ATHENA_SESSION_PLATFORM`` contextvar — that contextvar is
...
def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_athena_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_athena_dir(new_subpath, old_name)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Athena root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* ATHENA_HOME's cache, so a gateway running at
    the root (e.g. ``ATHENA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when ATHENA_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _ATHENA_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("ATHENA_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _ATHENA_ROOT
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    try:
        ...
        athena = athena_authorization_override("file.deliver", str(resolved))
    except Exception:
        athena = None
    if athena is not None:
        return str(resolved) if athena["allowed"] else None
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-ATHENA_HOME token takeover is armed by GatewayRunner only for
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-ATHENA_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Athena home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Athena home layout.
        """
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_athena_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # ATHENA_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile ATHENA_HOME isolation.
        """
