...
- GET  /v1/models                  — lists athena-agent and any configured model_routes aliases
...
def _athena_version() -> str:
    """Return the canonical Athena Agent version string.

    ``athena_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``athena_agent-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("ATHENA_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``athena-agent``)
    ...
    ``direct_model_requests`` config value here, while Athena-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Athena-aware and is
    always honored.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Athena-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`athena_cli.model_switch.resolve_effective_model` (the shared
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_athena_home()``, so each profile gets its own DB —
    ...
    def _create_agent(
        ...
        # athena_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list athena-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Athena provider/model inventory.
        ...
        Athena provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "athena.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="athena.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Athena-Session-Id header.  Without this,
