...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``athena_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ATHENA_HOME}/pairing`` while newer code/docs may
    write ``{ATHENA_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own ATHENA_HOME using the same legacy/consolidated layout rules
    as ``athena -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current ATHENA_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ATHENA_HOME
        ...
        if profile:
            ...
            # ``athena -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `athena` after
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``athena
