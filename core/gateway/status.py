...
from athena_constants import get_athena_home, _get_platform_default_athena_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_athena_home() / "gateway-starts.log"
...
def _get_process_athena_home() -> Path:
    """Return the process-level ATHENA_HOME, skipping context-local overrides.
    ...
    ``get_athena_home()`` honors ``_ATHENA_HOME_OVERRIDE`` contextvar used for
...
def _canonical_athena_home(path: Path | str) -> Path:
    """Return a stable absolute ATHENA_HOME path for persisted identity data."""
...
def _same_athena_home(left: Path | str, right: Path | str) -> bool:
    ...
    return os.path.normcase(str(_canonical_athena_home(left))) == os.path.normcase(
        str(_canonical_athena_home(right))
    )
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Athena' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Athena gateway lifecycle subcommand from a command line.
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `athena gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Athena-owned runtime records
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ATHENA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.athena`` or ``$ATHENA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``athena_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ATHENA_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a ATHENA_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"athena_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ATHENA_HOME= on the argv. ATHENA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "athena_home=" in command_lc and f"athena_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # ATHENA_HOME-local.  Persist the owning gateway's process home so an
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ATHENA_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``athena_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ATHENA_HOME.  Pass it when validating *another* profile's state file (the
...
def _get_takeover_marker_path(athena_home: Optional[Path] = None) -> Path:
    ...
    ``athena_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = athena_home or _get_process_athena_home()
    return _canonical_athena_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-ATHENA_HOME --replace while
    ...
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_athena_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_athena_home")
        if replacer_home is not None and not _same_athena_home(
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's ATHENA_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_athena_home(
            target_home or _get_process_athena_home()
        )
        ...
        record = {
            ...
            "target_athena_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_athena_home": str(
                _canonical_athena_home(_get_process_athena_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target ATHENA_HOME's gateway PID record, and the live
    ...
    if not isinstance(pid_record_home, str) or not _same_athena_home(
