...
agent's existing :func:`~athena_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``athena tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``athena tools``) → the quality-first default chain.
