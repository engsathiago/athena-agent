...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=athena``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`athena_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``athena tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from athena the next time the catalog is fetched, no patch required.
