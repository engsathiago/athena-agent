...
Optional Athena observability plugin that configures exporters and maps
Athena-specific observer hooks to NeMo Relay marks and ATIF state. Athena core
...
CLI UX. Athena core emits NeMo Relay lifecycle events for provider and tool
...
- Export the Relay scopes and LLM/tool lifecycles emitted by Athena core.
- Add Athena session, turn, approval, and subagent mark events.
...
wheel, Athena installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Athena shared client metrics.
...
Athena core routes provider and tool execution through NeMo Relay managed APIs
...
Athena uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Athena-owned
...
with this Athena-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Athena rejects
...
Athena activates these plugins before registering its managed LLM and tool
...
with Athena and a local Ollama model served through the OpenAI-compatible API.
...
- Athena core managed provider calls map to `llm` scope start/end events.
- Athena core managed tool calls map to `tool` scope start/end events.
...
Athena core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Athena-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Athena calls the
...
Athena provider call
  -> nemo_relay.llm.execute(...)
    -> Athena provider adapter callback(...)

Athena tool call
  -> nemo_relay.tools.execute(...)
    -> Athena authorization and dispatch callback(...)
```
