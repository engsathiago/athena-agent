...
_SESSION_INITIALIZER_NAME = "athena.nemo_relay.rich_observability"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Athena profile; keeping the existing process-global configuration "
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"athena.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"athena.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                ...
                state.atif_subscriber_name = (
                    f"athena.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Athena core Relay session is unavailable")
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Athena installs the managed execution
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Athena cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Athena-owned "
