...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.athena/.env over config.yaml.
    ...
    ATHENA_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
def register(ctx) -> None:
    ...
    # canonical home is the env var / ~/.athena/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "ATHENA_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
