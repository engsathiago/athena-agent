"""
A2A inbound platform adapter — exposes Athena as an A2A-discoverable agent.
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _build_card(self, public_url: Optional[str] = None, agent: Optional[dict] = None) -> dict:
        ...
        return protocol.build_agent_card(
            ...
            url=url,
            description=agent.get("description") or "Athena Agent — a general-purpose agent reachable over A2A.",
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed A2A task to another local Athena profile.
        ...
        with lock:
            ...
            cmd = ["athena", "chat", "-q", framed_text, "-Q", "--source", "a2a"]
            if session_id:
