...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Athena media cache directory.
    ...
    ``<kind>_cache`` layout that ``get_athena_dir`` may return.
    """
    try:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `athena whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `athena whatsapp`.",
                retryable=False,
            )
            return False
