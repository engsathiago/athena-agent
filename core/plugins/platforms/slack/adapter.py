...
# identify AthenaAgent traffic — matching other Athena outbound surfaces
# that already set ``AthenaAgent/<version>`` for platform-partner attribution.
try:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `athena slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `athena gateway setup` "
                "or in the active profile's ~/.athena/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `athena gateway setup` "
                "or add it to your active profile's ~/.athena/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `athena gateway setup` "
                "or in the active profile's ~/.athena/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `athena gateway setup` "
                "or add it to your active profile's ~/.athena/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ATHENA_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Athena has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Athena can "
            ...
            self._app.action(
                _re.compile(r"^athena_clarify_choice_\d+$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'athena slack manifest' if unsure), and (b) the other bot's "
    ...
    async def _handle_app_context_changed(
        ...
        # Athena owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_athena_no_thread_response"):
        ...
        # the shape produced by peer Athena agents in Socket Mode on some
    ...
    async def send_clarify(
        ...
        (unique ``athena_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``athena_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"athena_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "athena_clarify_other",
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_athena_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    print()
    print_info("   Full guide: see the Slack guide included with the Athena source")
    print()
