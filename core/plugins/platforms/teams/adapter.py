...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Athena owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="athena_approve",
            data={**btn_data_base, "athena_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="athena_approve",
                data={**btn_data_base, "athena_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="athena_approve",
                    data={**btn_data_base, "athena_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="athena_approve",
            data={**btn_data_base, "athena_action": "deny"}, style="destructive",
        ))
        body = [
