...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``ATHENA_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Athena response. Emoji-only acks are
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Athena has an LLM-generated session title and can safely rename
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Athena needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
...
def _component_check_auth(
    ...
    the pairing store (``athena pairing approve``).
    ...
    # so users approved via ``athena pairing approve`` can interact with
