...
#
# Out-of-process senders (cron subprocesses, `athena send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_athena_home() honors profile overrides — never hardcode ~/.athena.
...
# this module (athena status, test collection, plugin discovery).
...
# artifacts so Athena sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `athena photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `athena photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `athena setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, athena status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: athena photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: athena photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `athena update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``athena photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Athena does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `athena photon setup`, so the connect path must be able to
        ...
        # A `athena update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `athena send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`athena send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Athena gateway (which spawns the sidecar and "
                    "records its address under <athena-home>/runtime/"
