...
under an immutable ``/opt/athena`` (read-only for the athena user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ATHENA_HOME/photon/sidecar`` (the durable data volume,
