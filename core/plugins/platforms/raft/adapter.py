...
def interactive_setup() -> None:
    """Interactive ``athena gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Athena env
    ...
    print_info("Connect Athena to Raft as an external agent.")
