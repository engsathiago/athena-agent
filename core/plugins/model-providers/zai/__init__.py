...
request omits ``thinking``.  Athena' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Athena reasoning
...
(per Z.AI / BigModel docs).  Athena' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Athena reasoning effort onto GLM-5.2's native ``high``/``max``.
