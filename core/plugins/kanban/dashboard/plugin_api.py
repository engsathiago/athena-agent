    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                ...
                # backend can run under a different ATHENA_HOME than the
...
from athena_cli.kanban_db import (  # noqa: E402
...
def model_options():
    ...
    Thin wrapper around ``athena_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of Athena
...
def list_kanban_projects():
    """List first-class Athena projects for board scoping.
