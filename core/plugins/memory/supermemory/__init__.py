...
def _resolve_container_tag_for_setup(athena_home: str, *, identity: str = "default") -> str:
...
def _probe_supermemory_connection(api_key: str, athena_home: str, *, identity: str = "default") -> dict:
    ...
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(athena_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def post_setup(self, athena_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(Path(athena_home) / ".env", env_writes)
