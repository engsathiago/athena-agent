...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      2. Active profile name via profiles system -> ``athena_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"athena"`` (default profile)
    """
    ...
    # but applying it before profile resolution makes every named Athena
    # profile share that same host.  Keep named profiles isolated; only the
    # default Athena profile may opt into the config's default host.
