...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    healthy and ``athena memory status`` would stay green while the daemon
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Athena .env during
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def post_setup(self, athena_home: str, config: dict) -> None:
        ...
        # data-volume target instead of writing to /opt/athena (NS-605).
