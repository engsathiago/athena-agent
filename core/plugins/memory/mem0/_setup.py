...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # athena_cli/config.py. read_text() with no encoding falls back to the
...
def _setup_platform(athena_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.athena/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.athena/.env (or unset it) or Athena will keep "
...
def _setup_selfhosted(athena_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "athena-user"
    )
...
def _prompt_api_key(label: str, env_var: str, athena_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # athena_cli/config.py; a Notepad BOM on the first line would
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        try:
            ...
            # durable data-volume target instead of /opt/athena (NS-605).
...
def post_setup(athena_home: str, config: dict) -> None:
    ...
    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(athena_home, config, flags)
        return
