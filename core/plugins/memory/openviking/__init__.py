...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Athena to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Athena refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Athena did not start "
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        athena_home = str(kwargs.get("athena_home") or "").strip()
        if not athena_home:
            try:
                from athena_constants import get_athena_home
                athena_home = str(get_athena_home())
            except Exception:
                athena_home = str(Path.home() / ".athena")
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Athena will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.athena/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts athena (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Athena will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._athena_home:
            return None
        return Path(self._athena_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._athena_home:
            return None
        return Path(self._athena_home) / _RUN_LOCKS_RELATIVE_DIR
