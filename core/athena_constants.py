...
def _athena_home_from_env() -> Path:
    """Resolve ATHENA_HOME from the process environment only.

    Reads the ``ATHENA_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_athena_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_athena_home`
    and :func:`get_process_athena_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``ATHENA_HOME`` is
...
def get_athena_home() -> Path:
    ...
    :func:`set_athena_home_override`) → ``ATHENA_HOME`` env var → the
    ...
    ``athena_cli/kanban_db.py``).  See legacy upstream issue #18594.
    """
    ...
    return _athena_home_from_env()


def get_process_athena_home() -> Path:
    """Return the Athena home for the running process, ignoring task overrides.

    Unlike :func:`get_athena_home`, this never follows the context-local
    override set by :func:`set_athena_home_override`.  It resolves only the
    process ``ATHENA_HOME`` env var (falling back to the platform default),
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        3. ``<ATHENA_HOME>/skills`` last-resort
    """
...
def get_athena_dir(
    ...
    *,
    ...
    Args:
        ...
        home: Optional explicit Athena home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local ATHENA_HOME.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Athena-managed Node trees live under ``$ATHENA_HOME/node`` (or a profile's
    ``ATHENA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_athena_node_executable``
    used to trust file presence alone, so ``athena update`` would pick that
...
def _heal_managed_node_windows() -> bool:
    ...
    match = re.search(
        rf"node-v{_ATHENA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$ATHENA_HOME/node`` on POSIX.
    ...
    ``ATHENA_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_athena_managed_node() -> str | None:
    """Install a Athena-managed Node tree and return its npm path.
    ...
    Athena never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$ATHENA_HOME/node`` (the same tree a fresh install
...
def heal_athena_managed_node() -> bool:
    """Redownload Athena-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not athena_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ATHENA_HOME": str(get_athena_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_athena_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_ATHENA_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
...
def find_athena_node_executable(command: str) -> str | None:
    """Return a Athena-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_ATHENA_NODE_TARGET_MAJOR``) heal the
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Athena-managed installs.
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``athena update`` — leaving a **dangling symlink** that ``which`` still
...
def secure_parent_dir(path: Path) -> None:
    ...
    See legacy upstream issue #25821.
    """
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Athena itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: legacy upstream issue #47111
    """
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``athena_cli`` modules using three different Windows
    ...
    A few sites outside ``athena_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``athena_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Athena itself. An ImportError
...
# `athena_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Athena.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("athena_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `athena update` is exactly
    ...
    return [
        ...
        "    athena update",
        "If that also fails, run ./install.sh from a newer Athena source checkout.",
    ]
