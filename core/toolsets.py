...
_ATHENA_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on ATHENA_DESKTOP
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `athena tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        ...
        if include_registry and name.startswith("athena-"):
