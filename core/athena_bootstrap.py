...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``athena_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``athena_bootstrap``, never ``athena_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Athena ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Athena source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``ATHENA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
