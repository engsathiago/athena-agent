...
Extracted verbatim from athena_state.py so the SessionDB mixin modules
(athena_state_search / athena_state_schema / athena_state_portability) can
reference them without importing athena_state (which would be a cycle).
athena_state re-imports every name here for backward compatibility.
"""
...
# `athena sessions optimize-storage` rebuilds it on a capable host.
