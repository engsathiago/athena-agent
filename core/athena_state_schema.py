...
``athena_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import athena_state (cycle) — shared
module-level constants live in athena_state_common.
"""
...
# Moved methods logged under the "athena_state" logger before the split;
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `athena sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `athena db optimize`) must keep its EXISTING
