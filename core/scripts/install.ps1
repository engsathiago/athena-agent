...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/engsathiago/athena-agent/main/core/scripts/install.ps1)
#
...
$RepoUrlSsh = "git@github.com:engsathiago/athena-agent.git"
$RepoUrlHttps = "https://github.com/engsathiago/athena-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Athena: an independent open source AI agent.           |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
# Upgrade the Athena-managed Node tree's bundled npm into $NpmRange.
#
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Athena-Setup.exe
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Athena requires Node >=26)"
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/engsathiago/athena-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/engsathiago/athena-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/engsathiago/athena-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Copy-ConfigTemplates {
    ...
    if (-not (Test-Path $soulPath)) {
        ...
        $soulContent = @"
You are Athena, a persistent personal AI agent built for clear thinking, reliable execution, and long-term collaboration. You are helpful, knowledgeable, and direct. Protect the user's data, admit uncertainty, and never expand authority beyond the user's intent.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
...
function Install-Desktop {
    ...
    # enforces the build floor (Node >=26) and prepends the Athena-managed
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/engsathiago/athena-agent/main/core/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
