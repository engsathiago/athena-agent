...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/engsathiago/athena-agent/main/bootstrap.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:engsathiago/athena-agent.git"
REPO_URL_HTTPS="https://github.com/engsathiago/athena-agent.git"
...
print_banner() {
    ...
    echo "│             ⚕ Athena Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  Athena: an independent open source AI agent.           │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/engsathiago/athena-agent/main/core/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
# with the Athena-managed Node $NODE_VERSION.
...
check_node() {
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Athena requires Node >=26) — installing Athena-managed Node $NODE_VERSION..."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (athena --tui may not work)"
        }
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Athena-managed Node to PATH, so
