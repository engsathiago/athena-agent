...
unprivileged `athena` user, giving the layout most people have —
$ATHENA_HOME/athena-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  ATHENA_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .athena-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `athena setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${ATHENA_DEV_SANDBOX_UPSTREAM:-https://github.com/engsathiago/athena-agent.git}"
...
GIT_ROOT="${ATHENA_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t athena-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/github.com/engsathiago/athena-agent"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/github.com/engsathiago/athena-agent/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/github.com/engsathiago/athena-agent/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://raw.githubusercontent.com/engsathiago/athena-agent/main/bootstrap.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/athena-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `athena` launcher to ~/.local/bin and the
# checkout to $ATHENA_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/athena-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q athena-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t athena-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Athena sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/athena-sandbox/next"
