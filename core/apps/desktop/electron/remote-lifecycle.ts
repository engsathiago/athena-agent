...
const READY_RE = /^ATHENA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
async function locateAthena(ssh, remoteAthenaPath) {
  ...
  const err: any = new Error(
    'Athena is not installed on the remote host (could not find a `athena` executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/engsathiago/athena-agent/main/bootstrap.sh | sh  ' +
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, athenaPath = '') {
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.athenaPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(athenaPath, profile, opts: any = {}) {
  ...
  const dashCmd = `env ATHENA_DESKTOP=1 ${athena} ${profileArgs}${subCmd}`
...
async function spawnRemoteDashboard(ssh, { athenaPath, profile, token, ownershipId }) {
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(athenaPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
...
async function connect(deps) {
  ...
  if (lock) {
    ...
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.athenaPath))
