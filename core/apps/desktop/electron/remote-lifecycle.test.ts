...
test('locateAthena canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v athena/, '/home/u/.local/bin/athena\n'],
    [/\[ -x .*\.local\/bin\/athena/, 'OK'],
    [/python3 -c/, '/home/u/.athena/athena-agent/venv/bin/athena\n']
  ])

  assert.equal(await locateAthena(ssh, ''), '/home/u/.athena/athena-agent/venv/bin/athena')
})
