...
async function probeWindowsRemote(ssh, explicitAthenaPath = '') {
  ...
  const script = [
    ...
    '$candidates+=(Join-Path $athenaHome "athena-agent\\venv\\Scripts\\athena.exe")',
    '$candidates+=(Join-Path $HOME "athena-agent\\.venv\\Scripts\\athena.exe")',
    '$athena=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    ...
    '$python=Join-Path (Split-Path $athena) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Athena Python runtime was not found."}',
