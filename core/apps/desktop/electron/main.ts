...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Projeto Athena'
})
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged athena-setup or not — the resolver
      ...
      // `athena update` (backend) + `athena desktop --build-only` (OS-aware GUI
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // not just the `athena.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvAthena) || fileExists(path.join(updateRoot, '.athena-bootstrap-complete'))
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // PYTHONUNBUFFERED: `athena update` writes to a pipe here, so CPython
  ...
  // `athena update` reaps stale `athena serve` backends (a code update
...
async function waitForAthena(baseUrl, token, signal?, authMode?) {
...
async function discoverCloudAgents(org?: string) {
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Athena Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (athena #37247), and the gateway middleware transparently
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  return { ...connection, remoteAthenaVersion: result.athenaVersion || '' }
}
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAthena(remote.baseUrl, remote.token, undefined, remote.authMode)
...
async function startAthena() {
  ...
  const connectionPromise = (async () => {
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(athenaProcess, { readyFile }),
      backendStartFailed
    ])
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetAthenaConnection,
...
ipcMain.handle('athena:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))
...
ipcMain.handle('athena:find-in-page', (event, query, options) => {
...
ipcMain.handle('athena:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped athena_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(ATHENA_HOME, 'profiles', profile) : ATHENA_HOME
...
ipcMain.handle('athena:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
