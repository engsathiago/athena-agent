...
import { getAthenaConfigRecord, saveAthenaConfig } from '@/athena'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getAthenaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveAthenaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
