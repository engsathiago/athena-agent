...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  ...
  return window.athenaDesktop.api<T>({
...
export function pluginSocket(pluginId: string, path: string, onMessage: (data: unknown) => void): () => void {
  ...
  const connect = async () => {
    const connection = await window.athenaDesktop.getConnection().catch(() => null)
...
// "Error invoking remote method 'athena:api': Error: 404: ..." through the
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.athenaDesktop.api<SidebarSessionsResponse>({
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.athenaDesktop.api<CustomEndpointsResponse>({
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.athenaDesktop.api<CustomEndpointValidationResponse>({
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.athenaDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function authMcpServer(name: string): Promise<McpOAuthFlow> {
  return window.athenaDesktop.api<McpOAuthFlow>({
    ...profileScoped(),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.athenaDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.athenaDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getPairing(): Promise<PairingResponse> {
  return window.athenaDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.athenaDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.athenaDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.athenaDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.athenaDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.athenaDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.athenaDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.athenaDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
...
export function exportProfileArchive(
  ...
  return window.athenaDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.athenaDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.athenaDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.athenaDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.athenaDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.athenaDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.athenaDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.athenaDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.athenaDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.athenaDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.athenaDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.athenaDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function runDoctor(): Promise<ActionResponse> {
  return window.athenaDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.athenaDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.athenaDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.athenaDesktop.api<DebugShareResponse>({
