 ...
 * and `<athena home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<athena home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's athena_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // athena_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
