 ...
 *    `AthenaPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `athena-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<athena home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: AthenaPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
