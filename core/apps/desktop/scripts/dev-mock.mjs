...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `athena-dev-mock-${Date.now()}`))
  ...
  return { root, athenaHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(athenaHome, mockUrl) {
  fs.writeFileSync(
    path.join(athenaHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(athenaHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
